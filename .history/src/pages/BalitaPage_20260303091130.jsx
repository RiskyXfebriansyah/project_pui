...
export default function BalitaPage(props) {
  const { balita, onAddPemantauan, onAddBalita, onDelete, role, currentUser } = props;
  ...
  const columns = [
    ...
    { key: 'statusStunting', label: 'STATUS STUNTING',
      render: (v, r) => {
        // 1. Pakai dari API kalau sudah ada
        if (v) return <StatusBadge status={v}/>;
        // 2. Hitung dari tinggiBadan + umurBulan yang sudah ada di row (dari view)
        if (r.tinggiBadan && r.umurBulan) {
          return <StatusBadge status={getStatusStunting(r.tinggiBadan, r.umurBulan, r.jenisKelamin)}/>;
        }
        // 3. Hitung dari riwayat kalau ada
        if (r.riwayat?.length) {
          const last = r.riwayat[r.riwayat.length - 1];
          const umur = hitungUmurBulan(r.tanggalLahir);
          return <StatusBadge status={getStatusStunting(last.tinggiBadan || last.tb, umur, r.jenisKelamin)}/>;
        }
  ...
  async function savePemantauan() {
    ...
    // Hitung umur & status supaya tidak null di DB
    ...
    const dto = {
      ...
      tinggiBadan:   parseFloat(pmtForm.tb),
      lingkarKepala: parseFloat(pmtForm.lk),
      statusStunting,   // ← tambah ini
      statusGizi,       // ← tambah ini
    };
    const result = await onAddPemantauan(dto);
    setSaving(false);
  ...
  async function saveBalita() {
    ...
    orangTuaId:   currentUser?.role === 'orang_tua' ? currentUser.id : null,  // ← tambah ini
  };
    ...
    <div style={{ padding: 24 }}>
      ...
      <Modal open={!!detail && !showPmt} onClose={() => setDetail(null)}
        ...
        {detail && (() => {
          ...
          // Hitung status: API → tinggiBadan dari view → riwayat
          const ss = detail.statusStunting
            || (detail.tinggiBadan && detail.umurBulan
                ? getStatusStunting(detail.tinggiBadan, detail.umurBulan, detail.jenisKelamin)
                : last
                  ? getStatusStunting(last.tinggiBadan || last.tb, umur, detail.jenisKelamin)
                  : 'Belum diukur');
          const sg = detail.statusGizi
            || (detail.beratBadan && detail.umurBulan
                ? getStatusGizi(detail.beratBadan, detail.umurBulan, detail.jenisKelamin)
                : last
                  ? getStatusGizi(last.beratBadan || last.bb, umur, detail.jenisKelamin)
                  : 'Belum diukur');
