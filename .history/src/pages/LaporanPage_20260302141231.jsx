// ============================================================
//  LaporanPage.jsx — RESTRUCTURED
//  Struktur section sesuai form fisik:
//  I.   Informasi Umum Posyandu
//  II.  Kegiatan Penimbangan
//  III. Pemantauan ASI Eksklusif     ← ambil dari DB seperti Sec V
//  IV.  Pemantauan Balita Gizi Buruk & Kurus  ← NEW (sesuai form fisik)
//  V.   Formulir Pemantauan Pertumbuhan Balita ← dulunya Section IV
//
//  Perubahan:
//  - Sec III: sistem sama seperti Sec V (ambil dari DB, nama auto-fill)
//  - Sec III: tanggalLahir & umur konek otomatis dari DB
//  - Sec III: filter balita ≤12 bulan, jika belum ada → arahkan ke Data Balita
//  - Sec IV:  tabel baru Gizi Buruk & Kurus (Nama, Tgl Lahir, Umur, Nama Ortu, Pekerjaan, BB, TB)
//  - Sec V:   Formulir Pemantauan Pertumbuhan Balita (dulunya Sec IV)
//  - currentUser auto-fill Nama Posyandu & Petugas Lapangan
//  - defaultInfo di dalam komponen agar bisa akses currentUser
// ============================================================
...
// ── Modal Pilih Balita (shared: untuk Sec III, IV, V) ────────
function ModalPilihBalita({ title, subtitle, balitaList, onSelect, onClose, filterUmur, sudahDipilih }) {
  const [search, setSearch] = useState('');
  const [selectedIbu, setSelectedIbu] = useState(null);

  const filtered = balitaList.filter(b => {
    if (filterUmur !== undefined) {
      const umur = hitungUmur(b.tanggalLahir);
      if (umur > filterUmur) return false;
    }
  ...
  const anakDariIbu = selectedIbu ? balitaList.filter(b => {
    if (filterUmur !== undefined && hitungUmur(b.tanggalLahir) > filterUmur) return false;
    return b.namaIbu === selectedIbu;
  }) : [];
    ...
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center' }}
      ...
      <div style={{ background:'#fff', borderRadius:20, width:560, maxHeight:'85vh', overflow:'hidden', display:'flex', flexDirection:'column', boxShadow:'0 25px 60px rgba(0,0,0,0.2)' }}
        ...
        <div style={{ padding:'20px 24px', borderBottom:'1px solid #F0F0F0', flexShrink:0 }}>
          ...
          {filterUmur !== undefined && (
            <div style={{ marginTop:8, padding:'6px 10px', background:'#FFFBEB', borderRadius:8, fontSize:11, color:'#92400E', border:'1px solid #FDE68A' }}>
              ⚠️ Hanya menampilkan balita usia ≤{filterUmur} bulan
            </div>
          )}
        </div>
        ...
        <div style={{ padding:'16px 24px', flex:1, overflowY:'auto' }}>
              ...
              {ibuOptions.map((b,i) => (
                <div key={i} onClick={()=>setSelectedIbu(b.namaIbu)}
                  ...
                  <span style={{ fontSize:12, color:'#16A34A', fontWeight:700, background:'#F0FDF4', padding:'2px 10px', borderRadius:20 }}>
                    {balitaList.filter(x => x.namaIbu===b.namaIbu && (filterUmur===undefined || hitungUmur(x.tanggalLahir)<=filterUmur)).length} anak
                  </span>
                </div>
              ))}
            </div>
                  ...
                  <div key={b.id} style={{ padding:'14px', borderRadius:12, background:'#F9FAFB', marginBottom:10, border:'1px solid #E5E7EB' }}>
                    <div style={{ display:'flex', alignItems:'center', gap:12, marginBottom:8 }}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:11, color:'#9E9E9E' }}>{umur} bulan • {b.jenisKelamin}{b.nik?` • NIK: ${b.nik}`:''}</div>
                        {b.tanggalLahir && <div style={{ fontSize:11, color:'#6B7280' }}>Lahir: {b.tanggalLahir?.split('T')[0]}</div>}
                        {(b.beratBadan||b.tinggiBadan) && (
                          <div style={{ fontSize:11, color:'#16A34A', fontWeight:600 }}>📊 BB {b.beratBadan}kg • TB {b.tinggiBadan}cm</div>
                        )}
                        {b.statusStunting && (
                          <div style={{ fontSize:10, color:'#6B7280' }}>Status: <strong>{b.statusStunting}</strong>{b.statusGizi?` • Gizi: ${b.statusGizi}`:''}</div>
                        )}
                      </div>
                    </div>
                    {sudah ? (
                      <span style={{ fontSize:11, color:'#16A34A', fontWeight:700, padding:'6px 12px', background:'#F0FDF4', borderRadius:8, border:'1px solid #BBF7D0' }}>✅ Sudah ditambahkan</span>
                    ) : (
          ...
          {search && !selectedIbu && ibuOptions.length === 0 && (
            <div style={{ textAlign:'center', padding:'32px', color:'#9E9E9E' }}>
              ...
              <div style={{ fontSize:12, color:'#D97706', background:'#FFFBEB', padding:'10px 14px', borderRadius:8, marginTop:8, display:'inline-block' }}>
                💡 Pastikan balita sudah terdaftar di <strong>menu Data Balita</strong> terlebih dahulu
              </div>
            </div>
          )}
          ...
          {!search && (
            <div style={{ textAlign:'center', padding:'32px', color:'#9E9E9E' }}>
              ...
              <div style={{ fontSize:11, color:'#D97706', marginTop:8 }}>
                Jika tidak ditemukan, pastikan sudah diinput di <strong>menu Data Balita</strong>
              </div>
            </div>
          )}
        </div>
        ...
        <div style={{ padding:'12px 24px', borderTop:'1px solid #F0F0F0', flexShrink:0, display:'flex', justifyContent:'flex-end' }}>
          <button onClick={onClose} style={{ padding:'8px 18px', background:'#F3F4F6', border:'none', borderRadius:8, cursor:'pointer', fontFamily:'inherit', fontWeight:600, fontSize:13 }}>
            Tutup
          </button>
        </div>
      </div>
...
export default function LaporanPage({ balitaList = [], currentUser = null }) {
  ...
  // Section III — ASI Eksklusif (dari DB, ≤12 bln)
  const [asiRows, setAsiRows]           = useState([]);
  const [showModalAsi, setShowModalAsi] = useState(false);

  // Section IV — Gizi Buruk & Kurus (dari DB, semua umur)
  const [giziRows, setGiziRows]           = useState([]);
  const [showModalGizi, setShowModalGizi] = useState(false);

  // Section V — Pemantauan Pertumbuhan (dari DB)
  ...
  // ── Sync auto-fill jika currentUser async ─────────────────
  useEffect(() => {
    if (currentUser) {
      setInfo(prev => ({
        ...prev,
  ...
  async function handleFilterTanggal() {
    ...
    try {
      ...
      if (res?.status?.code === 200 && res.data) {
        ...
        if (d.giziRows?.length) {
          setGiziRows(d.giziRows.map(r => ({
            balitaId:   r.balitaId  || null,
            namaBalita: r.namaBalita|| '',
            tglLahir:   r.tglLahir  || '',
            umurBulan:  r.umurBulan || hitungUmur(r.tglLahir),
        ...
        setAsiRows([]); setGiziRows([]); setPemantauanRows([]);
        setIsSaved(false);
      }
  ...
  async function handleSimpan() {
    ...
    setSaving(true);
    try {
      const res = await LaporanAPI.simpan({ info, kegiatan, asiRows, giziRows, pemantauanRows });
  ...
  // ── Tambah ke Sec III (ASI) ────────────────────────────────
  function tambahAsi(balita) {
    if (asiRows.find(r => r.balitaId === balita.id)) {
      showError(`${balita.nama} sudah ada di daftar ASI`); return;
    }
    const umurBulan = hitungUmur(balita.tanggalLahir);
    ...
    showSuccess(`✅ ${balita.nama} (${umurBulan} bln) ditambahkan ke Sec. III`);
  }

  // ── Tambah ke Sec IV (Gizi Buruk) ─────────────────────────
  function tambahGizi(balita) {
    if (giziRows.find(r => r.balitaId === balita.id)) {
      showError(`${balita.nama} sudah ada di daftar Gizi Buruk`); return;
    }
    const umurBulan = hitungUmur(balita.tanggalLahir);
    setGiziRows(prev => [...prev, {
      balitaId:   balita.id,
      namaBalita: balita.nama                        || '',
      tglLahir:   balita.tanggalLahir?.split('T')[0] || '',
      umurBulan,
    ...
    showSuccess(`✅ ${balita.nama} ditambahkan ke Sec. IV`);
  }

  // ── Tambah ke Sec V (Pemantauan) ──────────────────────────
  function tambahBalita(balita) {
    ...
    setPemantauanRows(prev => [...prev, {
      balitaId:           balita.id,
      noKK:'', nik: balita.nik||'', anakKe:'',
      namaAnak:           balita.nama         || '',
      tglLahir:           balita.tanggalLahir || '',
      lp: balita.jenisKelamin==='Laki-laki'?'L': balita.jenisKelamin==='Perempuan'?'P':'',
      usiaKehamilanLahir:'', bbl:'', pbl:'', ukaLahir:'',
      namaOrtu:           balita.namaIbu   || '',
      nikAyah:'', noTlp: balita.noTelepon||'', alamat: balita.alamat||'', rt:'', rw:'',
      tglUkur: tglLalu, bb: bbLalu, pb: tbLalu,
      tglUkurBaru: new Date().toISOString().split('T')[0],
      bbBaru:'', pbBaru:'', lila:'', statusNTO:'', asiEksklusif:'',
      vitAFeb:'', vitAAgs:'', bukuKIA:'', ketPerkembangan:'', pkat:'', catatan:'',
      _bbLalu: bbLalu, _tbLalu: tbLalu, _tglUkurLalu: tglLalu,
      _statusStunting: balita.statusStunting||'', _statusGizi: balita.statusGizi||'',
    }]);
    showSuccess(`✅ ${balita.nama} ditambahkan ke Sec. V`);
  }

  function updateGiziRow(idx, field, val) {
    setGiziRows(prev => prev.map((r,i) => i===idx ? {...r,[field]:val} : r));
  }
  function updateRow(idx, field, val) {
    setPemantauanRows(prev => prev.map((r,i) => i===idx ? {...r,[field]:val} : r));
  }
  ...
  async function exportExcel() {
    ...
    try {
      ...
      ws1['!cols'] = [{wch:40},...Array(8).fill({wch:12})];
      ...
      const ws2 = XLSX.utils.aoa_to_sheet([
        ...
        [],
        ['No','No KK','NIK','Anak Ke','Nama Anak','Tgl Lahir','L/P','Usia Kmln','BBL','PBL','UKA','Nama Ortu','NIK Ayah','No Tlp','Alamat','RT','RW','Tgl Ukur','BB (kg)','TB/PB (cm)','LILA','N/T/O/B','ASI','VitA Feb','VitA Ags','Buku KIA','Ket Perkembangan','PKAT','Status Stunting'],
        ...pemantauanRows.map((r,i) => [
          i+1,r.noKK,r.nik,r.anakKe,r.namaAnak,
          r.tglLahir?formatTanggal(r.tglLahir):'',r.lp,r.usiaKehamilanLahir,r.bbl,r.pbl,r.ukaLahir,
          r.namaOrtu,r.nikAyah,r.noTlp,r.alamat,r.rt,r.rw,
          r.tglUkurBaru||r.tglUkur,r.bbBaru||r.bb,r.pbBaru||r.pb,
          r.lila,r.statusNTO,r.asiEksklusif,r.vitAFeb,r.vitAAgs,r.bukuKIA,r.ketPerkembangan,r.pkat,r._statusStunting||'',
        ]),
      ]);
  ...
  const tdSt = { padding:'4px 6px', borderBottom:'1px solid #F0F0F0', verticalAlign:'middle' };
  const numIn = { padding:'5px 4px', border:'1.5px solid #E5E7EB', borderRadius:6, fontSize:12, fontFamily:'inherit', outline:'none', width:52, textAlign:'center', background:'#fff' };
    ...
    <div style={{ padding:24, fontFamily:'inherit' }}>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:20 }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <div style={{ position:'relative' }}>
            <button onClick={exportExcel} style={{ padding:'8px 16px', background:isSaved?'#1B6B3A':'#9E9E9E', color:'#fff', border:'none', borderRadius:8, cursor:isSaved?'pointer':'not-allowed', fontFamily:'inherit', fontWeight:700, fontSize:13 }}>
              📊 Export Excel
            </button>
      ...
      {currentUser && (
        <div style={{ background:'#F0FDF4', border:'1.5px solid #BBF7D0', borderRadius:10, padding:'10px 16px', marginBottom:16, fontSize:12, color:'#15803D', display:'flex', alignItems:'center', gap:8 }}>
          <span>✨</span>
          <span>Nama Posyandu & Petugas Lapangan diisi otomatis dari akun <strong>{currentUser.nama}</strong> — bisa diedit manual jika perlu.</span>
        </div>
      )}
            ...
            <thead>
              <tr>
                ...
                {['0-5 L','0-5 P','6-11 L','6-11 P','12-23 L','12-23 P','24-60 L','24-60 P'].map(h=>(
                  <th key={h} style={{ ...thSt, minWidth:56 }}>{h} bln</th>
                ))}
              </tr>
            </thead>
            <tbody>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          <div>
            ...
            <p style={{ margin:'4px 0 0', fontSize:11, color:'#9E9E9E' }}>
              🔗 Data dari database balita • Hanya bayi ≤12 bulan • Umur dihitung otomatis dari tanggal lahir
            </p>
          </div>
          <Button onClick={()=>setShowModalAsi(true)}>➕ Tambah Bayi</Button>
        </div>

        {asiRows.length === 0 ? (
          <div style={{ textAlign:'center', padding:'32px 24px', color:'#9E9E9E' }}>
            <div style={{ fontSize:40, marginBottom:8 }}>🍼</div>
            <div style={{ fontWeight:600, marginBottom:4 }}>Belum ada data ASI Eksklusif</div>
            <div style={{ fontSize:12, marginBottom:8 }}>Klik "Tambah Bayi" untuk mencari dari database</div>
            <div style={{ fontSize:11, color:'#D97706', background:'#FFFBEB', padding:'8px 14px', borderRadius:8, display:'inline-block' }}>
              💡 Bayi belum muncul? Input dulu di menu <strong>Data Balita</strong>
            </div>
          </div>
        ) : (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              ...
              <tbody>
                {asiRows.map((r,i) => (
                  <tr key={i} style={{ background: i%2===0?'#F0FDF4':'#E8F8EF' }}>
                    <td style={{ ...tdSt, textAlign:'center', fontWeight:700 }}>
                      {i+1}
                      {r.balitaId && <div style={{ fontSize:8, color:'#16A34A' }}>🔗</div>}
                    </td>
                    {/* Nama & Tgl Lahir read-only dari DB */}
                    <td style={{ ...tdSt, fontWeight:600, whiteSpace:'nowrap', minWidth:140 }}>{r.namaBalita}</td>
                    <td style={{ ...tdSt, textAlign:'center', fontSize:11, color:'#6B7280', whiteSpace:'nowrap' }}>{r.tglLahir || '-'}</td>
                    {/* Umur otomatis dihitung */}
                    <td style={{ ...tdSt, textAlign:'center' }}>
                      <span style={{ fontWeight:700, color:'#1B6B3A', background:'#F0FDF4', padding:'2px 8px', borderRadius:12, fontSize:12 }}>
                        {r.umurBulan} bln
                      </span>
                    </td>
                    {['e0','e1','e2','e3','e4','e5','e6'].map(e=>(
                      <td key={e} style={{ ...tdSt, textAlign:'center' }}>
                        <input type="checkbox" checked={r[e]||false}
                          onChange={ev=>setAsiRows(p=>p.map((x,j)=>j===i?{...x,[e]:ev.target.checked}:x))}/>
                      </td>
                    ))}
                    <td style={{ ...tdSt, whiteSpace:'nowrap', minWidth:100 }}>{r.namaOrtu}</td>
                    <td style={tdSt}>
                      <button onClick={()=>setAsiRows(p=>p.filter((_,j)=>j!==i))}
                        style={{ background:'#FEF2F2', border:'none', borderRadius:6, padding:'4px 8px', cursor:'pointer', color:'#DC2626', fontSize:12 }}>🗑️</button>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Card style={{ marginBottom:16 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          <div>
            ...
            <p style={{ margin:'4px 0 0', fontSize:11, color:'#9E9E9E' }}>
              🔗 Data dari database balita • BB & TB diisi otomatis dari data ukur terakhir
            </p>
          </div>
          <Button onClick={()=>setShowModalGizi(true)}>➕ Tambah Balita</Button>
        </div>

        {giziRows.length === 0 ? (
          <div style={{ textAlign:'center', padding:'32px 24px', color:'#9E9E9E' }}>
            <div style={{ fontSize:40, marginBottom:8 }}>⚖️</div>
            <div style={{ fontWeight:600, marginBottom:4 }}>Belum ada data gizi buruk & kurus</div>
            <div style={{ fontSize:12 }}>Klik "Tambah Balita" untuk mencari dari database</div>
          </div>
        ) : (
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
              <thead>
                <tr>
                  {['No','Nama Balita','Tgl Lahir','Umur (bln)','Nama Ortu','Pekerjaan','BB (kg)','TB (cm)',''].map(h=>(
                    <th key={h} style={{ ...thSt, background:'#FFF7ED', color:'#D97706', borderBottom:'2px solid #FDE68A' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {giziRows.map((r,i) => (
                  <tr key={i} style={{ background:i%2===0?'#FFF7ED':'#FFFBEB' }}>
                    <td style={{ ...tdSt, textAlign:'center', fontWeight:700 }}>
                      {i+1}
                      {r.balitaId && <div style={{ fontSize:8, color:'#D97706' }}>🔗</div>}
                    </td>
                    <td style={{ ...tdSt, fontWeight:600, whiteSpace:'nowrap', minWidth:140 }}>{r.namaBalita}</td>
                    <td style={{ ...tdSt, textAlign:'center', fontSize:11, color:'#6B7280', whiteSpace:'nowrap' }}>{r.tglLahir || '-'}</td>
                    <td style={{ ...tdSt, textAlign:'center' }}>
                      <span style={{ fontWeight:700, color:'#D97706', background:'#FFF7ED', padding:'2px 8px', borderRadius:12, fontSize:12 }}>
                        {r.umurBulan} bln
                      </span>
                    </td>
                    <td style={{ ...tdSt, whiteSpace:'nowrap', minWidth:100 }}>{r.namaOrtu}</td>
                    <td style={tdSt}>
                      <input value={r.pekerjaan||''} onChange={e=>updateGiziRow(i,'pekerjaan',e.target.value)}
                        style={{ ...txtIn, width:110 }} placeholder="Pekerjaan..."/>
                    </td>
                    <td style={tdSt}>
                      <input type="number" step="0.01" value={r.bb||''} onChange={e=>updateGiziRow(i,'bb',e.target.value)}
                        style={numIn} placeholder="kg"/>
                    </td>
                    <td style={tdSt}>
                      <input type="number" step="0.1" value={r.tb||''} onChange={e=>updateGiziRow(i,'tb',e.target.value)}
                        style={numIn} placeholder="cm"/>
                    </td>
                    <td style={tdSt}>
                      <button onClick={()=>setGiziRows(p=>p.filter((_,j)=>j!==i))}
                        style={{ background:'#FEF2F2', border:'none', borderRadius:6, padding:'4px 8px', cursor:'pointer', color:'#DC2626', fontSize:12 }}>🗑️</button>
                    </td>
                  </tr>
                ))}
      ...
      <Card>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:14 }}>
          <div>
            ...
            <p style={{ margin:'4px 0 0', fontSize:11, color:'#9E9E9E' }}>
              🔗 Hijau = ter-link ke database • 🔵 Biru = data bulan lalu • 🟡 Kuning = input bulan ini
            </p>
          </div>
          ...
          <div style={{ overflowX:'auto' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:11, minWidth:1900 }}>
              <thead>
                <tr>
                  {[
                    {l:'No'},{l:'No KK'},{l:'NIK'},{l:'Anak Ke'},{l:'Nama'},{l:'Tgl Lahir'},{l:'L/P'},
                    {l:'Usia Kmln'},{l:'BBL'},{l:'PBL'},{l:'UKA'},{l:'Nama Ortu'},{l:'NIK Ayah'},{l:'No Tlp'},{l:'Alamat'},{l:'RT'},{l:'RW'},
                    {l:'Tgl Ukur Lalu',bg:'#EFF6FF',c:'#1D4ED8'},{l:'BB Lalu',bg:'#EFF6FF',c:'#1D4ED8'},{l:'TB Lalu',bg:'#EFF6FF',c:'#1D4ED8'},
                    {l:'Tgl Ukur *',bg:'#FFFDE7',c:'#B45309'},{l:'BB Baru *',bg:'#FFFDE7',c:'#B45309'},{l:'TB/PB Baru *',bg:'#FFFDE7',c:'#B45309'},
                    {l:'LILA'},{l:'N/T/O/B'},{l:'ASI'},{l:'VitA Feb'},{l:'VitA Ags'},{l:'Buku KIA'},{l:'Ket Perkembangan'},{l:'PKAT'},{l:''},
                  ].map((h,i) => (
              ...
              <tbody>
                {pemantauanRows.map((r,i) => (
                  <tr key={r.balitaId||i} style={{ background:r.balitaId?(i%2===0?'#F0FDF4':'#E8F8EF'):(i%2===0?'#fff':'#FAFAFA') }}>
                    <td style={{ ...tdSt, textAlign:'center', fontWeight:700 }}>
                      {i+1}{r.balitaId && <div style={{ fontSize:8, color:'#16A34A' }}>🔗</div>}
                    </td>
                    ...
                    <td style={{ ...tdSt, background:'#EFF6FF', textAlign:'center', fontSize:10, color:'#6B7280', whiteSpace:'nowrap' }}>{r._tglUkurLalu||r.tglUkur||'-'}</td>
                    <td style={{ ...tdSt, background:'#EFF6FF', textAlign:'center', fontWeight:700, color:'#1D4ED8' }}>{r._bbLalu||r.bb||'-'}</td>
                    <td style={{ ...tdSt, background:'#EFF6FF', textAlign:'center', fontWeight:700, color:'#1D4ED8' }}>{r._tbLalu||r.pb||'-'}</td>
                    ...
                    <td style={tdSt}>
                      <select value={r.ketPerkembangan||''} onChange={e=>updateRow(i,'ketPerkembangan',e.target.value)} style={{ ...selSt, minWidth:95 }}>
                        <option value="">-</option><option value="Sesuai">Sesuai</option><option value="Meragukan">Meragukan</option><option value="Penyimpangan">Penyimpangan</option>
                      </select>
                    </td>
                    <td style={tdSt}>
      ...
      {/* Modal Sec III: ASI — filter ≤12 bulan */}
      {showModalAsi && (
        <ModalPilihBalita
          title="Tambah Bayi ke Pemantauan ASI Eksklusif"
          subtitle="Data dari database balita — hanya bayi ≤12 bulan"
          balitaList={balitaList}
          filterUmur={12}
          sudahDipilih={asiRows}
          onSelect={b=>tambahAsi(b)}
          onClose={()=>setShowModalAsi(false)}
        />
      )}

      {/* Modal Sec IV: Gizi Buruk — semua umur */}
      {showModalGizi && (
        <ModalPilihBalita
          title="Tambah Balita ke Gizi Buruk & Kurus"
          subtitle="Data dari database balita — semua usia"
          balitaList={balitaList}
          sudahDipilih={giziRows}
          onSelect={b=>tambahGizi(b)}
          onClose={()=>setShowModalGizi(false)}
        />
      )}

      {/* Modal Sec V: Pemantauan Pertumbuhan */}
      {showAddBalita && (
        <ModalPilihBalita
          title="Tambah Balita ke Pemantauan Pertumbuhan"
          subtitle="Data dari database balita — semua usia"
          balitaList={balitaList}
          sudahDipilih={pemantauanRows}
          onSelect={b=>tambahBalita(b)}
          onClose={()=>setShowAddBalita(false)}
        />
      )}
    </div>
  );
