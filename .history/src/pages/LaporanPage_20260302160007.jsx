...
const defaultKegiatan = () => ({
  ...
  // 07: Ditimbang bulan ini (D) — manual
  d_L_0_5: 0,  d_P_0_5: 0,
  d_L_6_11: 0, d_P_6_11: 0,
  d_L_12_23: 0, d_P_12_23: 0,
  d_L_24_60: 0, d_P_24_60: 0,

  // 08: Tidak hadir (−) — manual
  m_L_0_5: 0,  m_P_0_5: 0,
  m_L_6_11: 0, m_P_6_11: 0,
  m_L_12_23: 0, m_P_12_23: 0,
  m_L_24_60: 0, m_P_24_60: 0,

  // 09: BGM per umur (kalau mau per kolom) atau cukup total
  bgm_L_0_5: 0,  bgm_P_0_5: 0,
  bgm_L_6_11: 0, bgm_P_6_11: 0,
  bgm_L_12_23: 0, bgm_P_12_23: 0,
  bgm_L_24_60: 0, bgm_P_24_60: 0,

  // 10: Vit A bayi Feb (per umur) – contoh
  vitAbayiFeb_L_0_5: 0,  vitAbayiFeb_P_0_5: 0,
  vitAbayiFeb_L_6_11: 0, vitAbayiFeb_P_6_11: 0,
  vitAbayiFeb_L_12_23: 0, vitAbayiFeb_P_12_23: 0,
  vitAbayiFeb_L_24_60: 0, vitAbayiFeb_P_24_60: 0,

  // 11: Vit A balita Feb (per umur)
  vitAbalitaFeb_L_0_5: 0,  vitAbalitaFeb_P_0_5: 0,
  vitAbalitaFeb_L_6_11: 0, vitAbalitaFeb_P_6_11: 0,
  vitAbalitaFeb_L_12_23: 0, vitAbalitaFeb_P_12_23: 0,
  vitAbalitaFeb_L_24_60: 0, vitAbalitaFeb_P_24_60: 0,

  // 09–12 versi total lama (kalau backend pakai nama ini)
  bawahGarisMerah: 0,
  vitA_bayi_feb: 0,
  vitA_bayi_ags: 0,
  vitA_balita_feb: 0,
...
async function handleSimpan() {
  ...
  // Normalkan pemRows → numeric / null sesuai PemantauanRowDto
  const pemRowsNormalized = pemRows.map(r => ({
    ...r,
    usiaKehamilanLahir:
      r.usiaKehamilanLahir === '' || r.usiaKehamilanLahir == null
        ? null
        : Number(r.usiaKehamilanLahir),

    bbl: r.bbl === '' || r.bbl == null ? null : Number(r.bbl),
    pbl: r.pbl === '' || r.pbl == null ? null : Number(r.pbl),
    ukaLahir: r.ukaLahir === '' || r.ukaLahir == null ? null : Number(r.ukaLahir),

    bb: r.bb === '' || r.bb == null ? null : Number(r.bb),
    pb: r.pb === '' || r.pb == null ? null : Number(r.pb),

    tglUkur: r.tglUkur || null,
    tglUkurBaru: r.tglUkurBaru || null,

    bbBaru: r.bbBaru === '' || r.bbBaru == null ? null : Number(r.bbBaru),
    pbBaru: r.pbBaru === '' || r.pbBaru == null ? null : Number(r.pbBaru),

    lila: r.lila === '' || r.lila == null ? null : Number(r.lila),

    asiEksklusif:
      r.asiEksklusif === '' || r.asiEksklusif == null
        ? null
        : Number(r.asiEksklusif),

    vitAFeb:
      r.vitAFeb === '' || r.vitAFeb == null ? null : Number(r.vitAFeb),
    vitAAgs:
      r.vitAAgs === '' || r.vitAAgs == null ? null : Number(r.vitAAgs),
    bukuKIA:
      r.bukuKIA === '' || r.bukuKIA == null ? null : Number(r.bukuKIA),
  }));
  ...
  setSaving(true);
  try {
    const res = await LaporanAPI.simpan({
      info,
    ...
    if (res?.status?.code === 200 || res?.status?.code === 201) {
      ...
      showError(res?.status?.message || 'Gagal menyimpan');
    }
  } catch {
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
      ...
      <tbody>
        {[
          ...
          <tr
            key={no}
            style={{ background: parseInt(no, 10) % 2 === 0 ? '#FAFAFA' : '#FFFFFF' }}
          >
            ...
            {fields.map((fieldKey, idx) => (
              ...
              >
                <input
                  ...
                  min="0"
                  value={
                    kegiatan[fieldKey] === 0 || kegiatan[fieldKey] == null
                      ? ''
                      : kegiatan[fieldKey]
                  }
                  onChange={e => {
                    ...
                    setKegiatan(prev => ({ ...prev, [fieldKey]: next }));
                  }}
                  style={numIn}
                />
              </td>
        ...
        <tr style={{ background: '#FFF7ED' }}>
          ...
          <td colSpan={8} style={{ ...td, textAlign: 'left', paddingLeft: 12 }}>
            <input
              type="number"
              min="0"
              value={kegiatan.bawahGarisMerah === 0 ? '' : kegiatan.bawahGarisMerah}
              onChange={e =>
                setKegiatan(p => ({
                  ...p,
                  bawahGarisMerah: e.target.value === '' ? 0 : Number(e.target.value),
                }))
              }
        ...
        <tr style={{ background: '#F0FDF4' }}>
          ...
          <td colSpan={8} style={{ ...td, textAlign: 'left', paddingLeft: 12 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <label style={{ fontSize: 11, color: '#6B7280', display: 'flex', gap: 6 }}>
                Februari
                <input
                  type="number"
                  min="0"
                  value={kegiatan.vitA_Bayi_Feb === 0 ? '' : kegiatan.vitA_Bayi_Feb}
                  onChange={e =>
                    setKegiatan(p => ({
                      ...p,
                      vitA_Bayi_Feb: e.target.value === '' ? 0 : Number(e.target.value),
                    }))
                  }
                  style={{ ...numIn, width: 70 }}
                />
              </label>
              <label style={{ fontSize: 11, color: '#6B7280', display: 'flex', gap: 6 }}>
                Agustus
                <input
                  type="number"
                  min="0"
                  value={kegiatan.vitA_Bayi_Ags === 0 ? '' : kegiatan.vitA_Bayi_Ags}
                  onChange={e =>
                    setKegiatan(p => ({
                      ...p,
                      vitA_Bayi_Ags: e.target.value === '' ? 0 : Number(e.target.value),
                    }))
                  }
                  style={{ ...numIn, width: 70 }}
                />
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
              <label style={{ fontSize: 11, color: '#6B7280', display: 'flex', gap: 6 }}>
                ...
                <input
                  type="number"
                  min="0"
                  value={kegiatan.vitA_Balita_Feb === 0 ? '' : kegiatan.vitA_Balita_Feb}
                  onChange={e =>
                    setKegiatan(p => ({
                      ...p,
                      vitA_Balita_Feb: e.target.value === '' ? 0 : Number(e.target.value),
                    }))
                  }
                  style={{ ...numIn, width: 70 }}
                />
              ...
              <label style={{ fontSize: 11, color: '#6B7280', display: 'flex', gap: 6 }}>
                ...
                <input
                  type="number"
                  min="0"
                  value={kegiatan.vitA_Balita_Ags === 0 ? '' : kegiatan.vitA_Balita_Ags}
                  onChange={e =>
                    setKegiatan(p => ({
                      ...p,
                      vitA_Balita_Ags: e.target.value === '' ? 0 : Number(e.target.value),
                    }))
                  }
                  style={{ ...numIn, width: 70 }}
                />
        ...
        <tr style={{ background: '#F0FDF4' }}>
          ...
          <td colSpan={8} style={{ ...td, textAlign: 'left', paddingLeft: 12 }}>
            <input
              type="number"
              min="0"
              value={kegiatan.jumlahHamil === 0 ? '' : kegiatan.jumlahHamil}
              onChange={e =>
                setKegiatan(p => ({
                  ...p,
                  jumlahHamil: e.target.value === '' ? 0 : Number(e.target.value),
                }))
              }
              style={{ ...numIn, width: 80 }}
            />
