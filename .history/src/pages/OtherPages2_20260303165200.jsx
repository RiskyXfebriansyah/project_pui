...
// - Toggle: semua balita / hanya yg punya ≥2 pengukuran
// - Tabel riwayat lengkap di bawah grafik (BB, TB, LK, status, catatan)
// - Multi-metrik: BB+TB sekaligus di satu grafik
// - Ringkasan statistik balita terpilih (trend BB naik/turun)
// ───────────────────────────────────────────────────────────────
...
function MiniDot({ status }) {
  ...
  return <span style={{ width: 8, height: 8, borderRadius: '50%', background: c.dot, display: 'inline-block', flexShrink: 0 }}/>;
}
...
export function PemantauanPage({ balitaList }) {
  ...
  const [showAll,    setShowAll]    = useState(false);    // semua atau ≥2 saja
  ...
  // ── style helpers ───────────────────────────────────────────
  const th = { padding: '8px 10px', fontSize: 10, fontWeight: 700, color: '#9E9E9E', background: '#F9FAFB', borderBottom: '1px solid #F0F0F0', textAlign: 'left', whiteSpace: 'nowrap' };
  const td = { padding: '8px 10px', borderBottom: '1px solid #F9FAFB', fontSize: 12, verticalAlign: 'middle' };
                    ...
                    return (
                      <button key={bl.id} onClick={() => setSelected(bl.id)} style={{
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: 12, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{bl.nama}</div>
                          <div style={{ fontSize: 10, color: '#9E9E9E' }}>{formatUmur(bl.tanggalLahir)} • {jmlUkur}x ukur</div>
                        </div>
                        {ss && (
                          <div style={{
              ...
              <Card>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
                  {[
                    ...
                    ['BB Terakhir', b.beratBadan ? `${b.beratBadan} kg` : (b.riwayat?.length ? `${b.riwayat[b.riwayat.length-1].bb || b.riwayat[b.riwayat.length-1].beratBadan} kg` : '-'), '#2563EB', '⚖️'],
                    ['TB Terakhir', b.tinggiBadan ? `${b.tinggiBadan} cm` : (b.riwayat?.length ? `${b.riwayat[b.riwayat.length-1].tb || b.riwayat[b.riwayat.length-1].tinggiBadan} cm` : '-'), '#7C3AED', '📐'],
                    ['Trend BB', trendBB?.label || '-', trendBB?.color || '#9E9E9E', '📈'],
                  ].map(([l, v, c, ico]) => (
                    <div key={l} style={{ padding: '12px', background: `${c}08`, borderRadius: 10, border: `1px solid ${c}18`, textAlign: 'center' }}>
                ...
                {grafikData.length < 1 ? (
                  ...
                  <div style={{
                    ...
                    border: '1px solid #FDE68A', fontSize: 12, color: '#D97706',
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                      ...
                      <tbody>
                        {[...b.riwayat].reverse().map((p, i, arr) => {
                          ...
                          // Status: dari DB dulu, fallback hitung
                          ...
                          return (
                            <tr key={p.id || i} style={{ background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                              <td style={{ ...td, color: '#9E9E9E', fontWeight: 600 }}>
                                {arr.length - i}
                              </td>
                              ...
                              <td style={td}>
                                ...
                                {bbDiff !== null && (
                                  <div style={{ fontSize: 10, color: bbDiff > 0 ? '#16A34A' : bbDiff < 0 ? '#DC2626' : '#D97706', fontWeight: 600 }}>
