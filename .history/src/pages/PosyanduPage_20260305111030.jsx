...
// Menerima props dari App.jsx — pola sama dengan JadwalPage
// ─────────────────────────────────────────────────────────────
...
export function PosyanduPage({
  ...
  isLoading    = false,
  error        = null,
  onAdd        = null,   // null → role bukan admin
  onDelete     = null,   // null → role bukan admin
  onRefresh,
  role         = 'kader',
}) {
  ...
  async function handleSubmit(e) {
    ...
    setSubmitting(false);
    if (res.ok) { setShowAdd(false); showToast('Posyandu berhasil ditambahkan'); }
    else setFormErr(res.msg);
  }
  ...
  async function handleDelete() {
    ...
    if (res.ok) showToast('Posyandu berhasil dihapus');
    ...
    <div style={{ padding: 24, fontFamily: font, minHeight: '100vh', background: C.bg }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16 }}>
              {posyanduList.map(p => (
                <PosyanduCard
                  ...
                  onDelete={() => setDeleteTarget({ id: p.id, nama: p.nama })}
                />
              ))}
            </div>
          )
      ...
      {showAdd && (
        <Overlay onClick={() => !submitting && setShowAdd(false)}>
          <Modal onClick={e => e.stopPropagation()}>
            <ModalHeader emoji="🏥" title="Tambah Posyandu" sub="Isi data posyandu baru"
              onClose={() => !submitting && setShowAdd(false)} />
            <form onSubmit={handleSubmit}>
              <Field label="Nama Posyandu *">
                <input style={inputSt} placeholder="Posyandu Melati"
                  value={form.nama} onChange={e => setForm(f => ({ ...f, nama: e.target.value }))} />
              </Field>
              <Field label="Desa *">
                <input style={inputSt} placeholder="Desa Sukamaju"
                  value={form.desa} onChange={e => setForm(f => ({ ...f, desa: e.target.value }))} />
              </Field>
              <Field label="Kecamatan *">
                <input style={inputSt} placeholder="Kec. Ciawi"
                  value={form.kecamatan} onChange={e => setForm(f => ({ ...f, kecamatan: e.target.value }))} />
              </Field>
              <Field label="Status">
                <select style={inputSt} value={form.aktif ? 'aktif' : 'nonaktif'}
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="button" onClick={() => setShowAdd(false)}
                  style={{ ...btnSecondary, flex: 1 }} disabled={submitting}>Batal</button>
                <button type="submit"
      ...
      {deleteTarget && (
        <Overlay onClick={() => !deleting && setDeleteTarget(null)}>
          <Modal onClick={e => e.stopPropagation()} maxW={380}>
            <div style={{ textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: C.muted, lineHeight: 1.65, marginBottom: 24 }}>
                ...
                Data balita yang terdaftar tidak ikut terhapus.
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                <button onClick={() => setDeleteTarget(null)}
                  style={{ ...btnSecondary, flex: 1 }} disabled={deleting}>Batal</button>
...
function Modal({ children, onClick, maxW = 440 }) {
  return (
    <div onClick={onClick} style={{
...
function ModalHeader({ emoji, title, sub, onClose }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22 }}>
      <div style={{ width: 44, height: 44, borderRadius: 12, background: C.sagePale,
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      <label style={{ display: 'block', fontSize: 11, fontWeight: 700,
        color: C.muted, marginBottom: 6, letterSpacing: 0.3 }}>{label}</label>
      {children}
    </div>
  );
}
...
function ErrBox({ msg }) {
  return (
    <div style={{ background: C.redPale, border: `1px solid ${C.redBd}`,
      borderRadius: 10, padding: '10px 14px', marginBottom: 16, color: C.red, fontSize: 13 }}>
      ⚠️ {msg}
    </div>
  );
}
...
const btnPrimary = {
  ...
  background: C.sage, color: '#fff', fontWeight: 700, fontSize: 13, cursor: 'pointer',
};
const btnSecondary = {
