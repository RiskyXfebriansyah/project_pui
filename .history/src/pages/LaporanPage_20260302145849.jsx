...
// Format tanggal: "01 Jan 2026" — handles ISO string & plain date string
function fmtTgl(raw) {
  if (!raw) return '-';
...
export default function LaporanPage({ balitaList = [], currentUser = null }) {
  ...
  function tambahPemDB(b) {
    ...
    // Nama Ortu = "Ibu: X / Ayah: Y" jika ada, atau salah satunya
  ...
  async function exportExcel() {
    ...
    try {
      ...
      const ws2=XLSX.utils.aoa_to_sheet([
        ...
        [`Posyandu: ${info.namaPosyandu}  •  Bulan: ${info.bulan} ${info.tahun}`],
        [],
        ['No','No KK','NIK','Anak Ke','Nama Anak','Tgl Lahir','L/P','Usia Kmln','BBL','PBL','UKA','Nama Ortu','NIK Ayah','No Tlp','Alamat','RT','RW','Tgl Ukur Lalu','BB Lalu','TB Lalu','Tgl Ukur Baru','BB Baru','TB/PB Baru','LILA','LIKA','N/T/O/B','ASI','VitA Feb','Buku KIA','Ket Perkembangan','PKAT','Status Stunting'],
        ...pemRows.map((r,i)=>[
  ...
  return (
    ...
    {/* ── II. Kegiatan Penimbangan ──────────────────────────── */}
      <Card style={{ marginBottom:16 }}>
          ...
          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
            <thead>
              <tr>
                ...
                {/* Header kelompok umur: gabungkan L+P */}
                {[['0-5 bln',2],['6-11 bln',2],['12-23 bln',2],['24-60 bln',2]].map(([h,s])=>(
                  <th key={h} colSpan={s} style={{ ...thG, textAlign:'center', borderLeft:'1px solid #E5E7EB' }}>{h}</th>
                ))}
              </tr>
              <tr>
                <th style={{ ...thG, background:'#F9FAFB' }}></th>
                <th style={{ ...thG, background:'#F9FAFB' }}></th>
                <th style={{ ...thG, background:'#F9FAFB' }}></th>
            ...
            <tbody>
              ...
              {/* 01–06: baris manual input */}
              {[
                ['01','Jumlah semua balita yang ada dikelompok penimbangan bulan ini','S',KODE.S.w,['s_L_0_5','s_P_0_5','s_L_6_11','s_P_6_11','s_L_12_23','s_P_12_23','s_L_24_60','s_P_24_60']],
                ['02','Jumlah balita yang terdaftar dan mempunyai KMS bulan ini','K',KODE.K.w,['k_L_0_5','k_P_0_5','k_L_6_11','k_P_6_11','k_L_12_23','k_P_12_23','k_L_24_60','k_P_24_60']],
                ['03','Jumlah balita yang naik berat badannya bulan ini','N',KODE.N.w,['n_L_0_5','n_P_0_5','n_L_6_11','n_P_6_11','n_L_12_23','n_P_12_23','n_L_24_60','n_P_24_60']],
                ['04','Jumlah balita yang tidak naik berat badannya bulan ini','T',KODE.T.w,['t_L_0_5','t_P_0_5','t_L_6_11','t_P_6_11','t_L_12_23','t_P_12_23','t_L_24_60','t_P_24_60']],
                ['05','Jumlah balita yang ditimbang bulan ini, tetapi tidak ditimbang bulan lalu','O',KODE.O.w,['o_L_0_5','o_P_0_5','o_L_6_11','o_P_6_11','o_L_12_23','o_P_12_23','o_L_24_60','o_P_24_60']],
                ['06','Jumlah balita yang baru pertama kali hadir di penimbangan bulan ini','B',KODE.B.w,['b_L_0_5','b_P_0_5','b_L_6_11','b_P_6_11','b_L_12_23','b_P_12_23','b_L_24_60','b_P_24_60']],
              ].map(([no, label, kode, color, fields]) => (
                <tr key={no} style={{ background: parseInt(no)%2===0 ? '#FAFAFA' : '#fff' }}>
                  <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>{no}</td>
                  <td style={{ ...tdL, color:'#374151', lineHeight:1.4 }}>{label}</td>
                  <td style={{ ...td }}>
                    <span style={{ fontWeight:800, fontSize:14, color, background:`${color}18`, padding:'2px 8px', borderRadius:6 }}>{kode}</span>
                  </td>
                  {fields.map((f,fi) => (
                    <td key={f} style={{ ...td, borderLeft: fi%2===0 ? '1px solid #E5E7EB' : undefined }}>
              ...
              {/* 07: D — auto-hitung N+T+O+B */}
              <tr style={{ background:'#EFF6FF' }}>
                <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>07</td>
                <td style={{ ...tdL, color:'#1D4ED8', fontWeight:600, lineHeight:1.4 }}>
                  Jumlah balita yang ditimbang bulan ini
                  <span style={{ display:'block', fontSize:10, color:'#93C5FD', fontWeight:400 }}>(03 + 04 + 05 + 06)</span>
                </td>
                <td style={{ ...td }}>
                  <span style={{ fontWeight:800, fontSize:14, color:'#1D4ED8', background:'#DBEAFE', padding:'2px 8px', borderRadius:6 }}>D</span>
                </td>
                {COLS.map((c,ci) => (
                  <td key={c} style={{ ...td, borderLeft: ci%2===0 ? '1px solid #BFDBFE' : undefined, fontWeight:700, color:'#1D4ED8', fontSize:13 }}>
                    {hitungD(kegiatan,c) !== '' ? hitungD(kegiatan,c) : <span style={{ color:'#CBD5E1' }}>—</span>}
                  </td>
                ))}
              </tr>
              ...
              {/* 08: (-) — auto-hitung K-D */}
              <tr style={{ background:'#EFF6FF' }}>
                <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>08</td>
                <td style={{ ...tdL, color:'#1D4ED8', fontWeight:600, lineHeight:1.4 }}>
                  Jumlah balita yang tidak hadir di penimbangan bulan ini
                  <span style={{ display:'block', fontSize:10, color:'#93C5FD', fontWeight:400 }}>(02 − 07)</span>
                </td>
                <td style={{ ...td }}>
                  <span style={{ fontWeight:800, fontSize:14, color:'#1D4ED8', background:'#DBEAFE', padding:'2px 8px', borderRadius:6 }}>(-)</span>
                </td>
                {COLS.map((c,ci) => (
                  <td key={c} style={{ ...td, borderLeft: ci%2===0 ? '1px solid #BFDBFE' : undefined, fontWeight:700, color:'#1D4ED8', fontSize:13 }}>
                    {hitungMinus(kegiatan,c) !== '' ? hitungMinus(kegiatan,c) : <span style={{ color:'#CBD5E1' }}>—</span>}
                  </td>
                ))}
              </tr>

              {/* 09: BGM — satu angka, colSpan */}
              <tr style={{ background:'#FFF7ED' }}>
                <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>09</td>
                <td style={{ ...tdL, color:'#374151' }}>Jumlah balita yang ada di bawah garis merah (BGM)</td>
                <td style={{ ...td }}>
                  <span style={{ fontWeight:800, fontSize:14, color:'#D97706', background:'#FEF3C7', padding:'2px 8px', borderRadius:6 }}>A</span>
                </td>
                <td colSpan={8} style={{ ...td, textAlign:'left', paddingLeft:12 }}>
                  <input type="number" min="0" value={kegiatan.bawahGarisMerah||''}
                    onChange={e=>setKegiatan(p=>({...p,bawahGarisMerah:e.target.value}))}
                    style={{ ...numIn, width:80 }}/>
                  <span style={{ fontSize:11, color:'#9E9E9E', marginLeft:8 }}>balita</span>
                </td>
              </tr>

              {/* 10: Vit A Bayi */}
              <tr style={{ background:'#F0FDF4' }}>
                <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>10</td>
                <td style={{ ...tdL, color:'#374151' }}>Jumlah bayi mendapat vitamin A bulan Februari/Agustus</td>
                <td style={{ ...td }}>
                  <span style={{ fontWeight:800, fontSize:14, color:'#16A34A', background:'#DCFCE7', padding:'2px 8px', borderRadius:6 }}>A</span>
                </td>
                <td colSpan={8} style={{ ...td, textAlign:'left', paddingLeft:12 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                    <label style={{ fontSize:11, color:'#6B7280', display:'flex', alignItems:'center', gap:6 }}>
                      Februari:
                      <input type="number" min="0" value={kegiatan.vitA_bayi_feb||''} onChange={e=>setKegiatan(p=>({...p,vitA_bayi_feb:e.target.value}))} style={{ ...numIn, width:70 }}/>
                    </label>
                    <label style={{ fontSize:11, color:'#6B7280', display:'flex', alignItems:'center', gap:6 }}>
                      Agustus:
                      <input type="number" min="0" value={kegiatan.vitA_bayi_ags||''} onChange={e=>setKegiatan(p=>({...p,vitA_bayi_ags:e.target.value}))} style={{ ...numIn, width:70 }}/>
                    </label>
                  </div>
                </td>
              </tr>
              ...
              {/* 11: Vit A Balita */}
              <tr style={{ background:'#F0FDF4' }}>
                <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>11</td>
                <td style={{ ...tdL, color:'#374151' }}>Jumlah balita mendapat vitamin A bulan Februari/Agustus</td>
                <td style={{ ...td }}>
                  <span style={{ fontWeight:800, fontSize:14, color:'#16A34A', background:'#DCFCE7', padding:'2px 8px', borderRadius:6 }}>A</span>
                </td>
                <td colSpan={8} style={{ ...td, textAlign:'left', paddingLeft:12 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                    <label style={{ fontSize:11, color:'#6B7280', display:'flex', alignItems:'center', gap:6 }}>
                      Februari:
                      <input type="number" min="0" value={kegiatan.vitA_balita_feb||''} onChange={e=>setKegiatan(p=>({...p,vitA_balita_feb:e.target.value}))} style={{ ...numIn, width:70 }}/>
                    </label>
                    <label style={{ fontSize:11, color:'#6B7280', display:'flex', alignItems:'center', gap:6 }}>
                      Agustus:
                      <input type="number" min="0" value={kegiatan.vitA_balita_ags||''} onChange={e=>setKegiatan(p=>({...p,vitA_balita_ags:e.target.value}))} style={{ ...numIn, width:70 }}/>
                    </label>
                  </div>
                </td>
              </tr>
              ...
              {/* 12: ASI Eksklusif */}
              <tr style={{ background:'#F0FDF4' }}>
                <td style={{ ...td, color:'#9E9E9E', fontSize:11, fontWeight:600 }}>12</td>
                <td style={{ ...tdL, color:'#374151' }}>Jumlah bayi dengan ASI Eksklusif pada bulan ini</td>
                <td style={{ ...td }}>
                  <span style={{ fontWeight:800, fontSize:14, color:'#16A34A', background:'#DCFCE7', padding:'2px 8px', borderRadius:6 }}>E</span>
                </td>
                <td colSpan={8} style={{ ...td, textAlign:'left', paddingLeft:12 }}>
                  <input type="number" min="0" value={kegiatan.aEksklusif||''}
                    onChange={e=>setKegiatan(p=>({...p,aEksklusif:e.target.value}))}
                    style={{ ...numIn, width:80 }}/>
                  <span style={{ fontSize:11, color:'#9E9E9E', marginLeft:8 }}>bayi</span>
                </td>
              </tr>
        ...
        {/* Keterangan legend bawah */}
        <div style={{ marginTop:16, paddingTop:14, borderTop:'1px solid #F0F0F0' }}>
          <div style={{ fontSize:11, fontWeight:700, color:'#6B7280', marginBottom:8 }}>📖 Keterangan Kode:</div>
          <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
            {Object.entries(KODE).map(([k,v])=>(
              <div key={k} style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 10px', background:v.bg, borderRadius:20, border:`1px solid ${v.w}22` }}>
                <span style={{ fontWeight:800, fontSize:13, color:v.w }}>{k}</span>
                <span style={{ fontSize:11, color:v.w, fontWeight:600 }}>= {v.label}</span>
              </div>
            ))}
            <div style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 10px', background:'#EFF6FF', borderRadius:20, border:'1px solid #BFDBFE' }}>
              <span style={{ fontWeight:800, fontSize:13, color:'#1D4ED8' }}>D</span>
              <span style={{ fontSize:11, color:'#1D4ED8', fontWeight:600 }}>= Ditimbang (03+04+05+06) ⚡ auto</span>
            </div>
            <div style={{ display:'flex', alignItems:'center', gap:6, padding:'5px 10px', background:'#EFF6FF', borderRadius:20, border:'1px solid #BFDBFE' }}>
              <span style={{ fontWeight:800, fontSize:13, color:'#1D4ED8' }}>(-)</span>
              <span style={{ fontSize:11, color:'#1D4ED8', fontWeight:600 }}>= Tidak hadir (02-07) ⚡ auto</span>
            </div>
          </div>

          {/* Keterangan ibu hamil & persalinan */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'0 20px', marginTop:16 }}>
            <div style={{ marginBottom:0 }}>
              <label style={{ display:'block', fontSize:11, fontWeight:600, color:'#6B7280', marginBottom:4 }}>V. Jumlah ibu hamil yang ada</label>
              <input type="number" min="0" value={kegiatan.jumlahHamil||''} onChange={e=>setKegiatan(p=>({...p,jumlahHamil:e.target.value}))}
                style={{ width:'100%', padding:'8px 12px', borderRadius:8, border:'1.5px solid #E5E7EB', fontSize:13, fontFamily:'inherit', outline:'none', boxSizing:'border-box' }}/>
            </div>
            <div style={{ marginBottom:0 }}>
              <label style={{ display:'block', fontSize:11, fontWeight:600, color:'#6B7280', marginBottom:4 }}>V. Jumlah persalinan bulan ini</label>
              <input type="number" min="0" value={kegiatan.jumlahPersalinan||''} onChange={e=>setKegiatan(p=>({...p,jumlahPersalinan:e.target.value}))}
                style={{ width:'100%', padding:'8px 12px', borderRadius:8, border:'1.5px solid #E5E7EB', fontSize:13, fontFamily:'inherit', outline:'none', boxSizing:'border-box' }}/>
            </div>
          </div>
        </div>
