...
async function handleSimpan() {
  ...
  // 1. Normalisasi kegiatan (sudah kamu lakukan)
  const kegiatanNormalized = Object.fromEntries(
    Object.entries(kegiatan).map(([key, value]) => {
      if (value === '' || value == null) return [key, 0];
      return [key, typeof value === 'number' ? value : Number(value)];
    })
  );

  // 2. Normalisasi baris pemantauan (khusus field yang harus angka)
  const pemRowsNormalized = pemRows.map(row => ({
    ...row,
    usiaKehamilanLahir:
      row.usiaKehamilanLahir === '' || row.usiaKehamilanLahir == null
        ? null               // karena di backend Nullable<int>
        : Number(row.usiaKehamilanLahir),

    bbl:
      row.bbl === '' || row.bbl == null ? null : Number(row.bbl),
    pbl:
      row.pbl === '' || row.pbl == null ? null : Number(row.pbl),

    // kalau ada field numeric lain di model backend, bereskan juga di sini,
    // misal bb, pb, lila, lika, vitAFeb, dll.
  }));

  // 3. (opsional) normalisasi giziRows kalau ada angka yang boleh kosong
  const giziRowsNormalized = giziRows.map(row => ({
    ...row,
    bb: row.bb === '' || row.bb == null ? null : Number(row.bb),
    tb: row.tb === '' || row.tb == null ? null : Number(row.tb),
  }));
