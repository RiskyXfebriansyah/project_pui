...
// FIX: selalu prioritaskan riwayat terbaru, bukan data upload pertama
function getLatestMeasurement(balita) {
  if (balita.riwayat?.length) {
    ...
    const sorted = [...balita.riwayat].sort((a, b) => {
      const dateA = new Date(a.tanggal || a.tglUkur || 0);
      const dateB = new Date(b.tanggal || b.tglUkur || 0);
      return dateB - dateA;
    });
...
export default function BalitaPage(props) {
  ...
  const columns = [
    ...
    { key: 'beratBadan', label: 'BB TERAKHIR',
      render: (v, r) => {
        // FIX: selalu ambil data terbaru dari riwayat
        const latest = getLatestMeasurement(r);
        if (!latest.bb) return <span style={{ color: '#BDBDBD', fontSize: 12 }}>-</span>;
        return (
          <div>
            <div style={{ fontWeight: 700, fontSize: 13 }}>{latest.bb} kg</div>
            <div style={{ fontSize: 11, color: '#9E9E9E' }}>{latest.tb} cm</div>
          </div>
        );
      }},
    { key: 'aksi', label: 'AKSI',
