...
function getLatestMeasurement(balita) {
  if (balita.riwayat?.length) {
    const sorted = [...balita.riwayat].sort((a, b) => {
      // Sort by id DESC — id terbesar = record paling baru di DB
      const idA = Number(a.id) || 0;
      const idB = Number(b.id) || 0;
      if (idB !== idA) return idB - idA;
      // Fallback: created_at atau tanggal
      const dateA = new Date(a.created_at || a.tanggal || a.tglUkur || 0);
      const dateB = new Date(b.created_at || b.tanggal || b.tglUkur || 0);
      return dateB - dateA;
    });
...
export default function BalitaPage(props) {
  ...
  const [inlineEdit, setInlineEdit] = useState({});      // ← NEW: inline edit per field { fieldKey: value }
  const [inlineSaving, setInlineSaving] = useState(null);// ← NEW: field yang sedang disimpan
  ...
  // ── Simpan satu field kosong (inline) ────────────────────────
  async function saveInlineField(fieldKey, value) {
    if (!value?.trim()) { showError('Field tidak boleh kosong'); return; }
    setInlineSaving(fieldKey);
    const dto = { [fieldKey]: value.trim() };
    const result = await onEditBalita(detail.id, dto);
    setInlineSaving(null);
    if (result?.ok || result === true) {
      showSuccess('Data berhasil disimpan!');
      setDetail(prev => ({ ...prev, [fieldKey]: value.trim() }));
      setInlineEdit(prev => { const n = { ...prev }; delete n[fieldKey]; return n; });
    } else {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* ── Modal Detail Balita ─────────────────────────────────── */}
      <Modal open={!!detail && !showPmt && !showEdit} onClose={() => setDetail(null)}
        ...
        {detail && (() => {
          ...
          return (
            <>
              ...
              {/* Info dasar — field kosong tampil tombol isi */}
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 20 }}>
                {[
                  ['NIK',       'nik',        detail.nik,        false],
                  ['Tgl Lahir', 'tanggalLahir', formatTanggal(detail.tanggalLahir), false],
                  ['Nama Ibu',  'namaIbu',    detail.namaIbu,    true],
                  ['Nama Ayah', 'namaAyah',   detail.namaAyah,   true],
                  ['Alamat',    'alamat',     detail.alamat,     true],
                  ['Telepon',   'noTelepon',  detail.noTelepon,  true],
                ].map(([label, fieldKey, val, editable]) => {
                  const isEmpty   = !val || val === '-';
                  const isEditing = inlineEdit[fieldKey] !== undefined;
                  const isSaving  = inlineSaving === fieldKey;
                  return (
                    <div key={fieldKey} style={{
                      background: isEmpty && editable ? '#FFFBEB' : '#F9FAFB',
                      borderRadius: 8, padding: '10px 12px',
                      border: isEmpty && editable ? '1.5px dashed #FCD34D' : '1.5px solid transparent',
                    }}>
                      <div style={{ fontSize: 10, color: '#9E9E9E', fontWeight: 600, marginBottom: 4 }}>{label}</div>
                      {isEditing ? (
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                          <input
                            autoFocus
                            value={inlineEdit[fieldKey]}
                            onChange={e => setInlineEdit(p => ({ ...p, [fieldKey]: e.target.value }))}
                            onKeyDown={e => {
                              if (e.key === 'Enter') saveInlineField(fieldKey, inlineEdit[fieldKey]);
                              if (e.key === 'Escape') setInlineEdit(p => { const n={...p}; delete n[fieldKey]; return n; });
                            }}
                            style={{
                              flex: 1, padding: '4px 8px', borderRadius: 6, border: '1.5px solid #1B6B3A',
                              fontSize: 12, fontFamily: 'inherit', outline: 'none',
                            }}
                          />
                          <button onClick={() => saveInlineField(fieldKey, inlineEdit[fieldKey])}
                            disabled={isSaving}
                            style={{
                              padding: '4px 10px', borderRadius: 6, border: 'none',
                              background: '#1B6B3A', color: '#fff', fontSize: 11,
                              fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit',
                              opacity: isSaving ? 0.7 : 1,
                            }}>
                            {isSaving ? '...' : '✓'}
                          </button>
                          <button onClick={() => setInlineEdit(p => { const n={...p}; delete n[fieldKey]; return n; })}
                            style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8 }}>
                          <span style={{
                            ...
                            color: isEmpty ? '#D97706' : 'inherit',
                            fontStyle: isEmpty ? 'italic' : 'normal',
                          }}>
                            {val || 'Belum diisi'}
                          </span>
                          {editable && onEditBalita && (
                            isEmpty ? (
                              <button
                                onClick={() => setInlineEdit(p => ({ ...p, [fieldKey]: '' }))}
                                style={{
                              ...
                              <button
                                onClick={() => setInlineEdit(p => ({ ...p, [fieldKey]: val }))}
                                style={{
                                  padding: '3px 8px', borderRadius: 20, border: 'none',
                                  background: 'transparent', color: '#9E9E9E', fontSize: 10,
                                  cursor: 'pointer', fontFamily: 'inherit',
                                }}>✏️</button>
                            )
                          )}
