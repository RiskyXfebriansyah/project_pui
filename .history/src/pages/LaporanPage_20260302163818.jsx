...
const defaultKegiatan = () => ({
  // 01 S
  s_L_0_5: 0,  s_P_0_5: 0,
  s_L_6_11: 0, s_P_6_11: 0,
  s_L_12_23: 0, s_P_12_23: 0,
  s_L_24_60: 0, s_P_24_60: 0,

  // 02 K
  k_L_0_5: 0,  k_P_0_5: 0,
  k_L_6_11: 0, k_P_6_11: 0,
  k_L_12_23: 0, k_P_12_23: 0,
  k_L_24_60: 0, k_P_24_60: 0,

  // 03 N
  n_L_0_5: 0,  n_P_0_5: 0,
  n_L_6_11: 0, n_P_6_11: 0,
  n_L_12_23: 0, n_P_12_23: 0,
  n_L_24_60: 0, n_P_24_60: 0,

  // 04 T
  t_L_0_5: 0,  t_P_0_5: 0,
  t_L_6_11: 0, t_P_6_11: 0,
  t_L_12_23: 0, t_P_12_23: 0,
  t_L_24_60: 0, t_P_24_60: 0,

  // 05 O
  o_L_0_5: 0,  o_P_0_5: 0,
  o_L_6_11: 0, o_P_6_11: 0,
  o_L_12_23: 0, o_P_12_23: 0,
  o_L_24_60: 0, o_P_24_60: 0,

  // 06 B
  b_L_0_5: 0,  b_P_0_5: 0,
  b_L_6_11: 0, b_P_6_11: 0,
  b_L_12_23: 0, b_P_12_23: 0,
  b_L_24_60: 0, b_P_24_60: 0,

  // Tambahan (nama harus sama dengan DTO KegiatanDto)
  bawahGarisMerah: 0,
  vitA_Bayi_Feb: 0,
  vitA_Bayi_Ags: 0,
  vitA_Balita_Feb: 0,
...
export default function LaporanPage({ balitaList = [], currentUser = null }) {
  ...
  // kalau di form kamu isi gram tapi DB pakai kg,
  // aktifkan ini khusus untuk berat lahir / bb:
  // num = num / 1000;

  if (num > max) num = max;
  if (num < -max) num = -max;
  return Math.round(num * 100) / 100; // 2 desimal
}
...
async function handleSimpan() {
  ...
  const pemRowsNormalized = pemRows.map(r => ({
    ...
    bbl: toDecimal(r.bbl, 999.99),
    pbl: toDecimal(r.pbl, 999.99),
    ukaLahir: toDecimal(r.ukaLahir, 99.99),
    bb: toDecimal(r.bb, 999.99),
    pb: toDecimal(r.pb, 999.99),
    bbBaru: toDecimal(r.bbBaru, 999.99),
    pbBaru: toDecimal(r.pbBaru, 999.99),
  ...
  setSaving(true);
  try {
    // ✅ KIRIM ID LAPORAN jika ada (untuk update)
    const payload = {
      id: laporanId, // <-- INI PENTING: kirim ID untuk update
      info,
      kegiatan: kegiatanNormalized,
      asiRows,
      giziRows: giziRowsNormalized,
    ...
    if (res?.status?.code === 200 || res?.status?.code === 201) {
      ...
      // Update laporanId jika response mengembalikan ID
      if (res.data?.id) {
        setLaporanId(res.data.id);
      }
    } else {
...
async function exportExcel() {
  // Hapus pengecekan isSaved agar bisa export kapan saja
  ...
  try {
    ...
    const wsData1 = [
      ...
      // II. KEGIATAN PENIMBANGAN - HEADER
      ['II. KEGIATAN PENIMBANGAN'],
      ['No', 'Keterangan', 'Kode', '0-5 bln', '', '6-11 bln', '', '12-23 bln', '', '24-60 bln', ''],
      ['', '', '', 'L', 'P', 'L', 'P', 'L', 'P', 'L', 'P'],
    ];

    // Data baris 01-12
    const barisKegiatan = [
      ['01', 'Jumlah semua balita yang ada dikelompok penimbangan bulan ini', 'S', 
        kegiatan.s_L_0_5 || 0, kegiatan.s_P_0_5 || 0,
        kegiatan.s_L_6_11 || 0, kegiatan.s_P_6_11 || 0,
        kegiatan.s_L_12_23 || 0, kegiatan.s_P_12_23 || 0,
        kegiatan.s_L_24_60 || 0, kegiatan.s_P_24_60 || 0],
      
      ['02', 'Jumlah balita yang terdaftar dan mempunyai KMS bulan ini', 'K',
        kegiatan.k_L_0_5 || 0, kegiatan.k_P_0_5 || 0,
        kegiatan.k_L_6_11 || 0, kegiatan.k_P_6_11 || 0,
        kegiatan.k_L_12_23 || 0, kegiatan.k_P_12_23 || 0,
        kegiatan.k_L_24_60 || 0, kegiatan.k_P_24_60 || 0],
      
      ['03', 'Jumlah balita yang naik berat badannya bulan ini', 'N',
        kegiatan.n_L_0_5 || 0, kegiatan.n_P_0_5 || 0,
        kegiatan.n_L_6_11 || 0, kegiatan.n_P_6_11 || 0,
        kegiatan.n_L_12_23 || 0, kegiatan.n_P_12_23 || 0,
        kegiatan.n_L_24_60 || 0, kegiatan.n_P_24_60 || 0],
      
      ['04', 'Jumlah balita yang tidak naik berat badannya bulan ini', 'T',
        kegiatan.t_L_0_5 || 0, kegiatan.t_P_0_5 || 0,
        kegiatan.t_L_6_11 || 0, kegiatan.t_P_6_11 || 0,
        kegiatan.t_L_12_23 || 0, kegiatan.t_P_12_23 || 0,
        kegiatan.t_L_24_60 || 0, kegiatan.t_P_24_60 || 0],
      
      ['05', 'Jumlah balita yang ditimbang bulan ini, tetapi tidak ditimbang bulan lalu', 'O',
        kegiatan.o_L_0_5 || 0, kegiatan.o_P_0_5 || 0,
        kegiatan.o_L_6_11 || 0, kegiatan.o_P_6_11 || 0,
        kegiatan.o_L_12_23 || 0, kegiatan.o_P_12_23 || 0,
        kegiatan.o_L_24_60 || 0, kegiatan.o_P_24_60 || 0],
      
      ['06', 'Jumlah balita yang baru pertama kali hadir di penimbangan bulan ini', 'B',
        kegiatan.b_L_0_5 || 0, kegiatan.b_P_0_5 || 0,
        kegiatan.b_L_6_11 || 0, kegiatan.b_P_6_11 || 0,
        kegiatan.b_L_12_23 || 0, kegiatan.b_P_12_23 || 0,
        kegiatan.b_L_24_60 || 0, kegiatan.b_P_24_60 || 0],
      
      [], // Spacer
      
      // Data tambahan
      ['07', 'Bawah Garis Merah (BGM)', 'A', kegiatan.bawahGarisMerah || 0],
      ['08', 'Jumlah Ibu Hamil', '-', kegiatan.jumlahHamil || 0],
      ['09', 'Jumlah Persalinan', '-', kegiatan.jumlahPersalinan || 0],
      ['10', 'Vitamin A Bayi (Februari)', 'A', kegiatan.vitA_Bayi_Feb || 0],
      ['11', 'Vitamin A Bayi (Agustus)', 'A', kegiatan.vitA_Bayi_Ags || 0],
      ['12', 'Vitamin A Balita (Februari)', 'A', kegiatan.vitA_Balita_Feb || 0],
      ['13', 'Vitamin A Balita (Agustus)', 'A', kegiatan.vitA_Balita_Ags || 0],
    ];

    // Gabungkan data
    barisKegiatan.forEach(row => wsData1.push(row));

    // ========== SHEET 2: ASI EKSKLUSIF ==========
    wsData1.push(
      [], // Spacer
      ['III. PEMANTAUAN ASI EKSKLUSIF'],
      ['No', 'Nama Balita', 'Tanggal Lahir', 'Umur (bln)', 'E0', 'E1', 'E2', 'E3', 'E4', 'E5', 'E6', 'Nama Orang Tua', 'Sumber Data']
    );
    ...
    wsData1.push(
      ...
      ['No', 'Nama Balita', 'Tanggal Lahir', 'Umur (bln)', 'Nama Orang Tua', 'Pekerjaan', 'BB (kg)', 'TB (cm)', 'Sumber Data']
    );
    ...
    ws1['!cols'] = [
      { wch: 5 },   // No
      { wch: 50 },  // Keterangan / Nama
      { wch: 8 },   // Kode
      { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, { wch: 8 }, // Kolom angka
      { wch: 25 },  // Nama Ortu
      { wch: 12 },  // Sumber
    ];

    // ========== SHEET 4: PEMANTAUAN PERTUMBUHAN ==========
    const wsData2 = [
      ...
      ['No', 'No KK', 'NIK', 'Anak Ke', 'Nama Anak', 'Tgl Lahir', 'L/P', 
       'Usia Kehamilan', 'BBL', 'PBL', 'UKA', 
       'Nama Ortu', 'NIK Ayah', 'No Telp', 'Alamat', 'RT', 'RW',
       'Tgl Ukur Lalu', 'BB Lalu', 'TB Lalu',
       'Tgl Ukur Baru', 'BB Baru', 'TB Baru',
       'LILA', 'LIKA', 'N/T/O/B', 'ASI Eksklusif', 'Vit A Feb', 'Buku KIA',
       'Ket Perkembangan', 'PKAT', 'Status Stunting']
    ];
    ...
    pemRows.forEach((r, i) => {
      wsData2.push([
        ...
        r.rt || '',
        r.rw || '',
        r._tglUkurLalu ? fmtTgl(r._tglUkurLalu) : (r.tglUkur ? fmtTgl(r.tglUkur) : ''),
        r._bbLalu || r.bb || '',
        r._tbLalu || r.pb || '',
        r.tglUkurBaru || '',
        r.bbBaru || '',
    ...
    // Set column widths untuk sheet 2
    ws2['!cols'] = [
      { wch: 5 },   // No
      { wch: 15 },  // No KK
      { wch: 18 },  // NIK
      ...
      { wch: 5 },   // RT
      { wch: 5 },   // RW
      { wch: 12 },  // Tgl Ukur Lalu
      { wch: 8 },   // BB Lalu
      { wch: 8 },   // TB Lalu
      { wch: 12 },  // Tgl Ukur Baru
      { wch: 8 },   // BB Baru
      { wch: 8 },   // TB Baru
      { wch: 8 },   // LILA
      { wch: 8 },   // LIKA
      { wch: 8 },   // N/T/O/B
      { wch: 10 },  // ASI Eksklusif
      { wch: 8 },   // Vit A Feb
      { wch: 8 },   // Buku KIA
      { wch: 12 },  // Ket Perkembangan
      { wch: 8 },   // PKAT
      { wch: 12 },  // Status Stunting
    ];

    // Apply border styles untuk semua sheet
    function applyBorderStyles(ws, range) {
      if (!ws['!ref']) return;
    ...
    // Apply borders
    applyBorderStyles(ws1);
    applyBorderStyles(ws2);

    // Merge cells untuk header utama
    ws1['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 10 } }, // Judul utama
      { s: { r: 1, c: 0 }, e: { r: 1, c: 10 } }, // Info posyandu
      { s: { r: 3, c: 0 }, e: { r: 3, c: 10 } }, // I. UMUM
    ];

    ws2['!merges'] = [
      { s: { r: 0, c: 0 }, e: { r: 0, c: 30 } }, // Judul utama
      { s: { r: 1, c: 0 }, e: { r: 1, c: 30 } }, // Info posyandu
    ];
    ...
    // Simpan file
    const fileName = `Laporan_${info.namaPosyandu || 'Posyandu'}_${info.bulan || 'Bulan'}_${info.tahun || 'Tahun'}.xlsx`;
    XLSX.writeFile(wb, fileName);
    
    showSuccess('✅ File Excel berhasil diunduh dengan format rapi!');
    ...
    <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
      <thead>
        <tr>
          ...
          <th style={{ ...thG, textAlign: 'left', paddingLeft: 10, background: '#F9FAFB', color: '#374151', minWidth: 280 }}>
            Keterangan
          </th>
          ...
          {[
            ...
            <th
              key={label}
              colSpan={span}
              style={{ ...thG, textAlign: 'center', borderLeft: '1px solid #E5E7EB' }}
            >
              {label}
            </th>
          ))}
        ...
        <tr>
          <th style={{ ...thG, background: '#F9FAFB' }} />
          <th style={{ ...thG, background: '#F9FAFB' }} />
          <th style={{ ...thG, background: '#F9FAFB' }} />
          {['L', 'P', 'L', 'P', 'L', 'P', 'L', 'P'].map((h, i) => (
            <th
              ...
              style={{
                ...thG,
                width: 52,
                borderLeft: i % 2 === 0 ? '1px solid #E5E7EB' : undefined,
              }}
            >
              {h}
            </th>
...
<tbody>
  {[
    [
      '01',
      'Jumlah semua balita yang ada dikelompok penimbangan bulan ini',
      'S',
      KODE.S.w,
      [
        's_L_0_5',
    ...
    [
      '06',
      'Jumlah balita yang baru pertama kali hadir di penimbangan bulan ini',
      'B',
      KODE.B.w,
      [
        'b_L_0_5',
    ...
    // 07–12: dibuat manual juga, bukan auto-hitungan
    [
      '07',
      'Jumlah balita yang ditimbang bulan ini (03 + 04 + 05 + 06)',
      'D',
      '#1D4ED8',
      [
        'd_L_0_5',
    ...
    [
      '08',
      'Jumlah balita yang tidak hadir di penimbangan bulan ini (02 − 07)',
      '-',
      '#1D4ED8',
      [
        'm_L_0_5',
    ...
    [
      ...
      [
        'vitAbalitaFeb_L_0_5',
        'vitAbalitaFeb_P_0_5',
        'vitAbalitaFeb_L_6_11',
        'vitAbalitaFeb_P_6_11',
        'vitAbalitaFeb_L_12_23',
        'vitAbalitaFeb_P_12_23',
        'vitAbalitaFeb_L_24_60',
        'vitAbalitaFeb_P_24_60',
      ],
    ],
    [
      '12',
    ...
    <tr
      ...
      style={{
        background:
          parseInt(no, 10) % 2 === 0 ? '#FAFAFA' : '#FFFFFF',
      }}
    >
      <td
        style={{
      ...
      {fields.map((fieldKey, idx) => (
        <td
          ...
          style={{
            ...td,
            borderLeft: idx % 2 === 0 ? '1px solid #E5E7EB' : undefined,
          }}
        >
          <input
            type="number"
            min="0"
            value={kegiatan[fieldKey] || ''}
            onChange={e =>
              setKegiatan(prev => ({
                ...prev,
