...
// ── FIX: cek isPast dengan gabungkan tanggal + jam selesai ────
// j.waktu format: "08:00-13:00" atau "08:00–13:00" atau "08:00"
// Ambil jam selesai (bagian kanan tanda - atau –)
// Jika tidak ada jam selesai, pakai akhir hari (23:59)
function hitungIsPast(tanggal, waktu) {
  if (!tanggal) return false;

  let jamSelesai = '23:59'; // default: akhir hari

  if (waktu) {
    // Pisah berdasarkan '-' atau '–' (en-dash)
    const parts = waktu.split(/[-–]/);
    if (parts.length >= 2) {
      jamSelesai = parts[parts.length - 1].trim(); // ambil bagian terakhir
    } else {
      jamSelesai = parts[0].trim(); // hanya 1 waktu → pakai itu
    }
  }
  ...
  const selesaiDate = new Date(`${tanggal}T${jamSelesai}:00`);
  return selesaiDate < new Date();
}
    ...
    <div style={{ padding:24 }}>
      ...
      {jadwal.length === 0 ? (
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(300px,1fr))', gap:14 }}>
          {jadwal.map(j => {
            // ── FIX: pakai hitungIsPast (tanggal + jam selesai) ──
            const isPast = hitungIsPast(j.tanggal, j.waktu);
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Tambah Jadwal" width={480}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:12 }}>
          ...
          <InputField label="Waktu" value={form.waktu} onChange={v => setForm(p=>({...p,waktu:v}))} placeholder="09:00-12:00"/>
