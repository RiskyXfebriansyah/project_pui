...
// ── JADWAL PAGE ────────────────────────────────────────────────
export function JadwalPage({ jadwal, onAdd, onDelete, role }) {
...
export function PenggunaPage({ users, onToggleAktif, onDelete, onAdd }) {
  const [tab, setTab]           = useState('medis');   // 'medis' | 'ortu'
  const [search, setSearch]     = useState('');
  const [showForm, setShowForm] = useState(false);
  const [formMedis, setFormMedis] = useState({nama:'',email:'',role:'bidan',posyandu:''});
  const [formOrtu,  setFormOrtu]  = useState({
    nama:'', email:'', noTelepon:'', namaAnak:'', posyandu:'', desa:''
  });
  ...
  function saveMedis() {
    if (!formMedis.nama || !formMedis.email) return;
    onAdd({ ...formMedis });
    setFormMedis({nama:'',email:'',role:'bidan',posyandu:''});
    setShowForm(false);
  }

  function saveOrtu() {
    if (!formOrtu.nama || !formOrtu.email) return;
    onAdd({ ...formOrtu, role:'orang_tua' });
    setFormOrtu({nama:'',email:'',noTelepon:'',namaAnak:'',posyandu:'',desa:''});
    setShowForm(false);
  }
  ...
  return (
    <div style={{padding:24}}>
      ...
      <Modal open={showForm && tab==='medis'} onClose={()=>setShowForm(false)}
        title="Tambah Tenaga Medis" width={420}>
        <InputField label="Nama Lengkap" value={formMedis.nama}
          onChange={v=>setFormMedis(p=>({...p,nama:v}))} required/>
        <InputField label="Email" value={formMedis.email}
          onChange={v=>setFormMedis(p=>({...p,email:v}))} type="email" required/>
        <SelectField label="Role" value={formMedis.role}
          onChange={v=>setFormMedis(p=>({...p,role:v}))}
          options={[
            {value:'admin', label:'Admin / Kepala Puskesmas'},
            {value:'bidan', label:'Bidan'},
            {value:'kader', label:'Kader Posyandu'},
          ]}/>
        <InputField label="Unit Kerja / Posyandu" value={formMedis.posyandu}
          onChange={v=>setFormMedis(p=>({...p,posyandu:v}))} placeholder="Mis: Posyandu Mawar"/>
        <div style={{
        ...
        <div style={{display:'flex',gap:10}}>
          <Button onClick={saveMedis}>💾 Simpan</Button>
          <Button variant="ghost" onClick={()=>setShowForm(false)}>Batal</Button>
        </div>
      </Modal>
      ...
      <Modal open={showForm && tab==='ortu'} onClose={()=>setShowForm(false)}
        ...
        <div style={{
          ...
          📱 Akun ini digunakan orang tua untuk login di <strong>aplikasi mobile</strong> dan memantau perkembangan anak.
        </div>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'0 14px'}}>
          <InputField label="Nama Orang Tua" value={formOrtu.nama}
            onChange={v=>setFormOrtu(p=>({...p,nama:v}))} required/>
          <InputField label="Email (untuk login)" value={formOrtu.email}
            onChange={v=>setFormOrtu(p=>({...p,email:v}))} type="email" required/>
          ...
          <InputField label="Posyandu" value={formOrtu.posyandu}
            onChange={v=>setFormOrtu(p=>({...p,posyandu:v}))} placeholder="Posyandu Mawar"/>
          <InputField label="Desa" value={formOrtu.desa}
            onChange={v=>setFormOrtu(p=>({...p,desa:v}))} placeholder="Sukamaju"/>
        </div>
        <div style={{
          padding:'10px 12px',background:'#FFFBEB',borderRadius:8,
          border:'1px solid #FDE68A',fontSize:12,color:'#92400E',marginBottom:14
        ...
        <div style={{display:'flex',gap:10}}>
          <Button onClick={saveOrtu}>💾 Daftarkan</Button>
          <Button variant="ghost" onClick={()=>setShowForm(false)}>Batal</Button>
        </div>
      </Modal>
    </div>
