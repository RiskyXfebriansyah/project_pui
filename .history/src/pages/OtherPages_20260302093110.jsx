...
export function PenggunaPage({ users, onToggleAktif, onDelete, onAdd }) {
  ...
  // ── Kirim ke API via onAdd (usePengguna.addPengguna) ─────────
  async function saveMedis() {
    if (!formMedis.nama || !formMedis.email) { alert('Nama dan email wajib diisi'); return; }
    setSaving(true);
  ...
  async function saveOrtu() {
    if (!formOrtu.nama || !formOrtu.email) { alert('Nama dan email wajib diisi'); return; }
    setSaving(true);
    const ok = await onAdd({
      nama:      formOrtu.nama,
  ...
  const colsMedis = [
    ...
    { key:'aktif',    label:'STATUS',
      render:(v,r)=>(
        <button onClick={()=>onToggleAktif(r.id, !r.aktif)} style={{
