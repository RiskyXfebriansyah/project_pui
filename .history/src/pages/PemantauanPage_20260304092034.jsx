...
//  Header balita → Tab BB/TB/LK → Area Chart → Nilai terakhir
//  → Trend badge → Tabel riwayat lengkap
// ============================================================
...
// ── warna per metrik ────────────────────────────────────────────
const METRICS = [
  { k: 'bb', l: 'BB (kg)', c: '#1B6B3A', fill: '#1B6B3A', light: '#F0FDF4', badge: '#DCFCE7' },
  { k: 'tb', l: 'TB (cm)', c: '#2563EB', fill: '#2563EB', light: '#EFF6FF', badge: '#DBEAFE' },
  { k: 'lk', l: 'LK (cm)', c: '#D97706', fill: '#D97706', light: '#FFFBEB', badge: '#FEF3C7' },
];
...
function CustomTooltip({ active, payload, label, metric }) {
  ...
  return (
    <div style={{
      background: '#fff', border: `1.5px solid ${m?.c || '#E5E7EB'}`,
...
// ── Trend Badge (mirip mobile) ──────────────────────────────────
function TrendBadge({ label, diff, unit }) {
          ...
          {!b ? (
            ...
            <>
              ...
              <div style={{
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    {bStatus.stunting && (
                      <div style={{
                        ...
                        background: statusColor(bStatus.stunting).bg,
                        color: statusColor(bStatus.stunting).text
                      }}>{bStatus.stunting}</div>
                    )}
                    {bStatus.gizi && (
                      <div style={{
                        padding: '4px 10px', borderRadius: 20, fontSize: 11, fontWeight: 700,
                        background: statusColor(bStatus.gizi).bg,
                        color: statusColor(bStatus.gizi).text
                      }}>{bStatus.gizi}</div>
                    )}
                  </div>
                </div>
                ...
                {/* ── Nilai Terakhir: BB / TB / LK ─────────── */}
                <div style={{
                  display: 'grid', gridTemplateColumns: '1fr 1fr 1fr',
                  gap: 0, borderTop: '1px solid #F5F5F5',
                  borderBottom: '1px solid #F5F5F5'
                }}>
                  {METRICS.map((m, i) => (
                    <div
                      key={m.k}
                      onClick={() => setActiveKey(m.k)}
                      style={{
                    ...
                    >
                      ...
                      <div style={{ fontSize: 10, color: '#9E9E9E', marginTop: 2, fontWeight: 600 }}>
                        {m.l.split(' ')[0]}
                      </div>
                    </div>
                  ))}
                ...
                {(trends.bb !== null || trends.tb !== null || trends.lk !== null) && (
                  <div style={{ display: 'flex', gap: 8, padding: '12px 20px', flexWrap: 'wrap' }}>
                    {trends.bb !== null && <TrendBadge label="BB" diff={trends.bb} unit="kg"/>}
                    {trends.tb !== null && <TrendBadge label="TB" diff={trends.tb} unit="cm"/>}
                    {trends.lk !== null && <TrendBadge label="LK" diff={trends.lk} unit="cm"/>}
                    <span style={{ fontSize: 10, color: '#BDBDBD', alignSelf: 'center' }}>
                      vs pengukuran sebelumnya
                    </span>
                  </div>
                )}
              </div>
                        ...
                        {[...b.riwayat].reverse().map((p, i, arr) => {
                          ...
                          return (
                            <tr key={p.id || i} style={{ background: i % 2 === 0 ? '#fff' : '#FAFAFA' }}>
                              ...
                              <td style={td}>
                                {pss
                                  ? <span style={{ padding: '3px 8px', borderRadius: 6, fontSize: 10, fontWeight: 700, background: pssC.bg, color: pssC.text }}>{pss}</span>
                                  : <span style={{ color: '#D1D5DB' }}>—</span>
                                }
                              </td>
                              <td style={td}>
                                {psg
                                  ? <span style={{ padding: '3px 8px', borderRadius: 6, fontSize: 10, fontWeight: 700, background: psgC.bg, color: psgC.text }}>{psg}</span>
                                  : <span style={{ color: '#D1D5DB' }}>—</span>
                                }
                              </td>
                              <td style={{ ...td, color: '#6B7280', maxWidth: 160, fontSize: 11 }}>
                                {p.catatan || <span style={{ color: '#E5E7EB' }}>—</span>}
                              </td>
                            </tr>
                          );
                        })}
