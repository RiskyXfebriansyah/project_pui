...
export default function LaporanPage({ balitaList = [], currentUser = null }) {
 ...
 async function handleSimpan() {
  ...
  // 🔹 Normalisasi semua field kegiatan → angka
  const kegiatanNormalized = Object.fromEntries(
    Object.entries(kegiatan).map(([key, value]) => {
      if (value === '' || value == null) {
        return [key, 0];              // kosong jadi 0
      }
      if (typeof value === 'number') {
        return [key, value];          // sudah number, biarkan
      }
      // kalau string angka ("1","2",dst) ubah ke Number
  ...
  try {
    const res = await LaporanAPI.simpan({
      info,
      kegiatan: kegiatanNormalized,   // ⬅ pakai yang sudah dibersihkan
      asiRows,
      giziRows,
      pemantauanRows: pemRows,
    });
