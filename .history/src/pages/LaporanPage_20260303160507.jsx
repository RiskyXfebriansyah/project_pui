...
function isToday(dateStr) {
  if (!dateStr) return true; // belum load = data baru = today
    ...
    <div style={{
      ...
      <div>
        ...
        <div style={{ fontSize:11, color:'#B45309', marginTop:2 }}>
          Data ini sudah dikunci. Tambah balita, simpan, dan hapus tidak tersedia untuk data hari sebelumnya.
          Gunakan tanggal hari ini untuk input baru.
        </div>
      </div>
    </div>
  );
...
export default function LaporanPage({ balitaList = [], currentUser = null, onRefresh }) {
  ...
  // ── VIEW ONLY: true jika data yang dimuat bukan hari ini ─────
  ...
  async function handleFilterTanggal() {
    ...
    try {
      ...
      // Tentukan apakah view-only (tanggal bukan hari ini)
      const viewOnly = !isToday(filterTanggal);
  ...
  function tambahAsiDB(b) {
    ...
    setAsiRows(p=>[...p,{
      ...
      e0:false,e1:false,e2:false,e3:false,e4:false,e5:false,e6:false, namaOrtu:b.namaIbu||'',
    }]);
    setIsSaved(false);
