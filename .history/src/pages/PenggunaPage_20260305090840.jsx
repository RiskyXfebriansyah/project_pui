...
function Toast({ toast }) {
  ...
  return (
    <div style={{ position:'fixed', top:24, left:'50%', transform:'translateX(-50%)', zIndex:99999, padding:'14px 24px', borderRadius:14, background:toast.type==='success'?'#16A34A':'#DC2626', color:'#fff', fontWeight:700, fontSize:14, boxShadow:'0 8px 32px rgba(0,0,0,0.2)', display:'flex', alignItems:'center', gap:10, fontFamily:'inherit' }}>
...
function getToken() {
  try { return JSON.parse(localStorage.getItem('posyandu_user') || '{}').token || ''; }
  catch { return ''; }
}

export default function PenggunaPage({ users, onToggleAktif, onDelete, onAdd, onRefresh }) {
  ...
  // ── Save orang tua + balita via endpoint CreateOrtu ───────
  async function saveOrtu() {
    ...
    setSaving(true);
    try {
      const res = await fetch(`${BASE_URL}/api/Pengguna/CreateOrtu`, {
        method: 'POST',
  ...
  const colsOrtu = [
    ...
    { key:'namaAnak', label:'NAMA ANAK', render: v => (
      <span style={{ padding:'4px 10px', borderRadius:20, background:v?'#F0FDF4':'#F9FAFB', fontSize:12, fontWeight:600, color:v?'#16A34A':'#9E9E9E' }}>
        {v ? `👶 ${v}` : 'Belum terhubung'}
      </span>
    )},
  ...
  return (
    <div style={{ padding:24 }}>
      ...
      <Card>
        ...
        {tab==='ortu' && (
          <div style={{ padding:'10px 14px', background:'#F5F3FF', borderRadius:10, border:'1px solid #DDD6FE', marginBottom:14, fontSize:12, color:'#6D28D9' }}>
      ...
      <Modal open={showForm && tab==='ortu'} onClose={()=>setShowForm(false)} title="Daftarkan Akun Orang Tua" width={500}>
        ...
        <div style={{ margin:'16px 0 12px', borderTop:'1.5px dashed #E5E7EB', position:'relative' }}>
          <span style={{ position:'absolute', top:-10, left:'50%', transform:'translateX(-50%)', background:'#fff', padding:'0 10px', fontSize:11, fontWeight:700, color:'#9E9E9E' }}>
            👶 Data Anak (opsional)
          </span>
        </div>
