...
// Field yang WAJIB ada di setiap item balitaList:
//   id, nik, nama, jenisKelamin, tanggalLahir
//   namaIbu, namaAyah, noTelepon, alamat   ← dari tabel balita snake_case → camelCase
//   beratBadan, tinggiBadan                ← dari vw_balita_lengkap (ukuran terakhir)
//   tglUkurTerakhir                        ← dari vw_balita_lengkap
//   statusStunting, statusGizi             ← dari vw_balita_lengkap
//   namaPosyandu, desa                     ← dari JOIN posyandu
//   riwayat: []                            ← array pemantauan (opsional)
export default function LaporanPage({ balitaList = [] }) {
  ...
  async function handleFilterTanggal() {
    ...
    try {
      ...
      if (res?.status?.code === 200 && res.data) {
        ...
        // Pemantauan rows — preserve balitaId + data bulan lalu dari SP JOIN
        if (d.pemantauanRows?.length) {
          setPemantauanRows(d.pemantauanRows.map(r => ({
            ...
            alamat:             r.alamat             || '',
            rt:r.rt||'', rw:r.rw||'',
            // Data laporan bulan ini (yang sudah disimpan)
            tglUkur:            r.tglUkur            || '',
            bb:                 r.bb                 || '',
            pb:                 r.pb                 || '',
            // Field baru untuk input ulang bulan depan (dikosongkan saat load)
            ...
            pkat:               r.pkat               || '',
            catatan:            '',
            // Data master dari SP JOIN vw_balita_lengkap (prefix _ = read-only cache)
  ...
  // ── Tambah ke Section IV (Pemantauan Pertumbuhan) ─────────────
  // Semua field identitas dari tabel balita (via useBalita hook)
  // Data bulan lalu dari vw_balita_lengkap (sudah include di hook)
  function tambahBalita(balita) {
    ...
    // Fallback: cek riwayat jika field view belum ada
    ...
    setPemantauanRows(prev => [...prev, {
      balitaId:           balita.id,          // FK ke tabel balita — WAJIB
      ...
      usiaKehamilanLahir: '',
      bbl:'', pbl:'', ukaLahir:'',
      // Dari master balita (camelCase — sudah di-map di hook)
      namaOrtu:           balita.namaIbu       ||'',
      nikAyah:            '',
      noTlp:              balita.noTelepon     ||'',
      alamat:             balita.alamat        ||'',
      rt:'', rw:'',
      // Data bulan lalu (tampilan read-only referensi — warna biru)
      tglUkur:            tglLalu,
      bb:                 bbLalu,
      pb:                 tbLalu,
      // Data baru bulan ini (diisi user — warna kuning)
      tglUkurBaru:        new Date().toISOString().split('T')[0],
      bbBaru:'', pbBaru:'', lila:'',
      statusNTO:'', asiEksklusif:'',
      vitAFeb:'', vitAAgs:'', bukuKIA:'',
      ketPerkembangan:'', pkat:'', catatan:'',
      // Cache read-only dari view (prefix _ tidak dikirim ke API)
      _bbLalu:            bbLalu,
      _tbLalu:            tbLalu,
  ...
  // ── Tambah ke Section III (ASI Eksklusif) ─────────────────────
