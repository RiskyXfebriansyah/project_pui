...
export default function BalitaPage(props) {
  ...
  async function saveBalita() {
    if (!form.nama || !form.tanggalLahir) {
      showError('Nama dan tanggal lahir wajib diisi');
      return;
    }
    if (!form.posyanduId) {
      showError('ID Posyandu wajib diisi');
      return;
    }
    setSaving(true);
    const dto = {
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* ── Modal Tambah Balita ─────────────────────────────────── */}
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Tambah Balita Baru" width={520}>

        <div style={{
          padding: '10px 14px', background: '#FFFBEB', borderRadius: 10,
          border: '1px solid #FDE68A', fontSize: 12, color: '#92400E', marginBottom: 16
        }}>
          ℹ️ Field dengan <strong>*</strong> wajib diisi dan dikirim ke API
        </div>

        {/* Field yang dikirim ke API (BalitaCreateDTO) */}
        <div style={{
