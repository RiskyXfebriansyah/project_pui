...
// shorthands
const E    = () => cell('', { bdr:false });                                                                     // empty no border
const EB   = () => cell('', { bdr:true, bg:WHT });                                                              // empty with border
...
const PLN  = (v,x={}) => cell(v, { bdr:false, sz:10, ...x });                                                   // plain (no border)
...
function sheet1(info, keg, asi, gizi) {
  ...
  rows.push([TIT('CATATAN BULANAN KEGIATAN PENIMBANGAN DI POSYANDU')]); M(0,0,0,TC-1);
  rows.push([TIT(`Posyandu: ${info.namaPosyandu||'-'}   |   Bulan: ${info.bulan||'-'} ${info.tahun||''}`,{sz:11})]); M(1,0,1,TC-1);
  rows.push(BL());

  rows.push([SEC('I. INFORMASI UMUM POSYANDU')]); M(rows.length-1,0,rows.length-1,TC-1);
  [
    ...
    const bG = { style:'thin', color:{ rgb:'CCCCCC' } };
    const mCell = (first) => ({
      v:'', t:'s',
      s:{
        ...
        border:{ top:bT, bottom:bT, left:first?bT:bG, right:bT },
      }
    });
    rows.push([LBL(l), VAL(String(v),{ha:'center'}), ...Array(TC-2).fill(0).map((_,i)=>mCell(i===0))]);
    M(ri, 2, ri, TC-1);
  });
  rows.push(BL());

  rows.push([SEC('II. KEGIATAN PENIMBANGAN')]); M(rows.length-1,0,rows.length-1,TC-1);
  ...
  [
  ...
  ].forEach(([no,lbl,kode,...ff],idx)=>{
    const vals=ff.map(f=>num(keg[f]));
    ...
    rows.push([
      ...
      ...vals.map(v=>NUM(v,{bg})),
      NUM(tL,{bg,bold:true}), NUM(tP,{bg,bold:true}),
  ...
  // Sec III ASI
  rows.push([SEC('III. PEMANTAUAN ASI EKSKLUSIF')]); M(rows.length-1,0,rows.length-1,TC-1);
  rows.push(['No','Nama Balita','Tanggal Lahir','Umur (bln)','E0','E1','E2','E3','E4','E5','E6','Nama Orang Tua','Sumber'].map(h=>HDR(h)));
  if(!asi.length){
    const r=rows.length; rows.push([DAT('-',{ha:'center'}),DAT('Belum ada data'),...Array(TC-2).fill(E())]); M(r,1,r,TC-1);
  } else {
    asi.forEach((r,i)=>{
      const bg=i%2===1?GRN5:WHT;
      rows.push([cell(i+1,{ha:'center',bg,sz:9,bdr:true}),DAT(r.namaBalita||'-',{bg,bold:true}),DAT(fmtTgl(r.tglLahir),{ha:'center',bg}),NUM(r.umurBulan,{bg}),...['e0','e1','e2','e3','e4','e5','e6'].map(e=>DAT(r[e]?'v':'',{ha:'center',bg})),DAT(r.namaOrtu||'-',{bg}),DAT(r.balitaId?'Database':'Manual',{ha:'center',bg})]);
    });
  }
  rows.push(BL());

  // Sec IV Gizi
  rows.push([SEC('IV. PEMANTAUAN BALITA GIZI BURUK & KURUS')]); M(rows.length-1,0,rows.length-1,TC-1);
  ...
  if(!gizi.length){
    ...
    gizi.forEach((r,i)=>{
      const bg=i%2===1?GRN5:WHT;
      rows.push([cell(i+1,{ha:'center',bg,sz:9,bdr:true}),DAT(r.namaBalita||'-',{bg,bold:true}),DAT(fmtTgl(r.tglLahir),{ha:'center',bg}),NUM(r.umurBulan,{bg}),DAT(r.namaOrtu||'-',{bg}),DAT(r.pekerjaan||'-',{bg}),DAT(String(r.bb||''),{ha:'center',bg}),DAT(String(r.tb||''),{ha:'center',bg}),DAT(r.balitaId?'Database':'Manual',{ha:'center',bg})]);
    });
  }
...
// ══════════════════════════════════════════════════════════════
// Set print settings: landscape, fit to 1 page wide, A4
function setPrintLandscape(ws, totalCols) {
  // ── Page setup: LANDSCAPE, A4, fit to width ──────────────
  // Catatan: xlsx-js-style memerlukan format khusus agar
  // orientation terbaca oleh Excel saat Ctrl+P
  ws['!pageSetup'] = {
    orientation:  'landscape',  // <-- HORIZONTAL
    paperSize:    9,            // 9 = A4
    fitToPage:    true,
    fitToWidth:   1,            // fit 1 halaman lebar
    fitToHeight:  0,            // tinggi bebas
    horizontalDpi: 300,
    verticalDpi:   300,
  };
  // Margin dalam inches
...
export async function exportLaporanExcel({ info, kegiatan, asiRows, giziRows, pemRows, extraData={} }) {
  ...
  const ws1 = sheet1(info,kegiatan,asiRows,giziRows);
  const ws2 = sheet2(info,pemRows,extraData);
  const ws3 = sheet3(info,kegiatan,asiRows,giziRows,pemRows);

  // Set semua sheet: LANDSCAPE A4
  [ws1, ws2, ws3].forEach(setPrintLandscape);
  ...
  // ── Tulis sebagai array buffer lalu patch XML pageSetup ──
  // xlsx-js-style kadang tidak inject <pageSetup> dengan benar,
  // jadi kita patch binary ZIP secara langsung setelah write.
  try {
    // Coba tulis dengan bookType xlsx dan inject pageSetup via sheet XML
    const wbOut = XLSX.write(wb, { bookType:'xlsx', type:'array', compression:true });
    
    // Patch: setelah file dibuat, buka dengan JSZip jika tersedia
    // dan inject pageSetup landscape ke setiap sheet XML
    if (window.JSZip) {
      await patchXlsxLandscape(wbOut, fn);
    } else {
      // Fallback: load JSZip lalu patch
      await loadJSZip();
      await patchXlsxLandscape(wbOut, fn);
    }
  } catch(e) {
    console.warn('Patch landscape gagal, fallback ke writeFile:', e);
...
async function loadJSZip() {
  ...
  return new Promise((res, rej) => {
    ...
    s.onerror = () => rej(new Error('JSZip gagal load'));
    document.head.appendChild(s);
  });
}

// Patch XML di dalam file .xlsx untuk force LANDSCAPE + FIT TO PAGE
async function patchXlsxLandscape(arrayBuf, fileName) {
  ...
  for (const sheetFile of sheetFiles) {
    let xml = await zip.files[sheetFile].async('string');

    // 1. Hapus semua tag yang akan kita ganti
    xml = xml.replace(/<sheetPr[^>]*\/>/g, '');
    xml = xml.replace(/<sheetPr[^>]*>.*?<\/sheetPr>/gs, '');
    xml = xml.replace(/<pageSetup[^>]*\/>/g, '');
    xml = xml.replace(/<pageSetup[^>]*>.*?<\/pageSetup>/gs, '');
    xml = xml.replace(/<pageMargins[^>]*\/>/g, '');
    xml = xml.replace(/<printOptions[^>]*\/>/g, '');

    // 2. Inject <sheetPr> dengan fitToPage=1 SEBELUM <dimension> atau <sheetViews>
    //    Ini yang membuat Excel tahu scaling = "Fit to Page"
    ...
    if (!xml.includes('<sheetPr>') && !xml.includes('<sheetPr ')) {
      ...
      // Insert setelah tag pembuka <worksheet ...>
      xml = xml.replace(/(<worksheet[^>]*>)/, `$1${sheetPrXml}`);
    }

    // 3. Inject pageSetup landscape + fit to width sebelum </worksheet>
    //    paperSize=9 = A4, orientation=landscape
    //    fitToWidth=1 fitToHeight=0 = fit to 1 page wide, unlimited height
    const pageMarginXml = `<pageMargins left="0.25" right="0.25" top="0.5" bottom="0.5" header="0.2" footer="0.2"/>`;
    const pageSetupXml  = `<pageSetup paperSize="9" orientation="landscape" fitToWidth="1" fitToHeight="0" horizontalDpi="300" verticalDpi="300"/>`;
    const printOptXml   = `<printOptions horizontalCentered="1"/>`;

    xml = xml.replace('</worksheet>',
      `${printOptXml}${pageMarginXml}${pageSetupXml}</worksheet>`
    );
