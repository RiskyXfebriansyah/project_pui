...
export async function exportLaporanExcel({ info, kegiatan, asiRows, giziRows, pemRows, extraData={} }) {
  ...
  // Buat sheets TANPA !pageSetup — biar tidak konflik dengan JSZip patch
  ...
  try {
    // Tulis ke ArrayBuffer dulu
    const wbOut = XLSX.write(wb, { bookType:'xlsx', type:'array' });

    // Load JSZip kalau belum ada
    if (!window.JSZip) await loadJSZip();

    // Patch XML landscape di dalam ZIP
    await patchXlsxLandscape(wbOut, fn);
  } catch(e) {
    console.warn('Patch gagal, fallback XLSX.writeFile:', e);
...
// Patch XML — inject LANDSCAPE + FIT TO PAGE ke setiap sheet
async function patchXlsxLandscape(arrayBuf, fileName) {
  ...
  for (const sf of sheetFiles) {
    // ── Baca sebagai Uint8Array lalu decode manual (hindari BOM issue) ──
    const u8  = await zip.files[sf].async('uint8array');
    // Strip UTF-8 BOM (EF BB BF) kalau ada
    const raw = (u8[0] === 0xEF && u8[1] === 0xBB && u8[2] === 0xBF)
      ? u8.slice(3) : u8;
    let xml = new TextDecoder('utf-8').decode(raw);

    // ── Bersihkan tag lama ────────────────────────────────────────────
    xml = xml.replace(/<sheetPr\b[^>]*\/>/g, '');
    xml = xml.replace(/<sheetPr\b[^>]*>[\s\S]*?<\/sheetPr>/g, '');
    xml = xml.replace(/<pageSetup\b[^>]*\/>/g, '');
    xml = xml.replace(/<pageSetup\b[^>]*>[\s\S]*?<\/pageSetup>/g, '');
    xml = xml.replace(/<pageMargins\b[^>]*\/>/g, '');
    xml = xml.replace(/<printOptions\b[^>]*\/>/g, '');

    // ── Inject <sheetPr fitToPage="1"> pakai indexOf (bukan regex) ───
    const wsStart  = xml.indexOf('<worksheet');
    const wsTagEnd = xml.indexOf('>', wsStart);
    if (wsStart !== -1 && wsTagEnd !== -1) {
      xml = xml.slice(0, wsTagEnd + 1)
        + '<sheetPr><pageSetUpPr fitToPage="1"/></sheetPr>'
        + xml.slice(wsTagEnd + 1);
    }

    // ── Inject pageSetup landscape sebelum </worksheet> ──────────────
    const closeIdx = xml.lastIndexOf('</worksheet>');
    if (closeIdx !== -1) {
      xml = xml.slice(0, closeIdx)
        + '<pageMargins left="0.25" right="0.25" top="0.5" bottom="0.5" header="0.2" footer="0.2"/>'
        + '<pageSetup paperSize="9" orientation="landscape" fitToWidth="1" fitToHeight="0"/>'
        + '</worksheet>';
    }

    // ── Tulis kembali sebagai UTF-8 bytes (tanpa BOM) ────────────────
    zip.file(sf, new TextEncoder().encode(xml));
  }
