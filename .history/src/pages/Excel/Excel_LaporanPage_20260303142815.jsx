...
export async function exportLaporanExcel({ info, kegiatan, asiRows, giziRows, pemRows, extraData={} }) {
  ...
  try {
    ...
    if (window.JSZip) {
      await patchXlsxLandscape(wbOut, fn, XLSX);
    } else {
...
// Patch XML di dalam file .xlsx untuk force landscape
async function patchXlsxLandscape(arrayBuf, fileName, XLSX) {
  const JSZip = window.JSZip;
  const zip   = await JSZip.loadAsync(arrayBuf);

  // Sheet XML ada di xl/worksheets/sheet1.xml, sheet2.xml, dst
  const sheetFiles = Object.keys(zip.files).filter(f => f.match(/^xl\/worksheets\/sheet\d+\.xml$/));
  
  // pageSetup XML yang di-inject (landscape, A4, fit to 1 page wide)
  const pageSetupXml = `<pageSetup paperSize="9" orientation="landscape" fitToWidth="1" fitToHeight="0" r:id="rId1"/>`;
  ...
  for (const sheetFile of sheetFiles) {
    ...
    // Hapus pageSetup lama jika ada
    xml = xml.replace(/<pageSetup[^/]*\/>/g, '');
    xml = xml.replace(/<pageSetup[^>]*>[^<]*<\/pageSetup>/g, '');
    xml = xml.replace(/<pageMargins[^/]*\/>/g, '');
    xml = xml.replace(/<printOptions[^/]*\/>/g, '');
    
    // Inject sebelum </worksheet>
