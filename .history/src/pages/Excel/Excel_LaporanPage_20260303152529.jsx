...
//
//  KOLOM BARU (26 total, A=0 … Z=25):
//  A(0)  = Label kiri (Provinsi, Kab, dst)
//  B(1)  = Titik dua kiri
//  C(2)  = Nilai kiri — anchor merge C–H (D–H = col 3–7, merge ke C)
//     merge
//  I(8)  = Label tengah (Nama Posyandu, Alamat, dst)
//  J(9)  = Titik dua tengah
//  K(10) = Nilai tengah — anchor merge K–Q (L–Q = col 11–16, merge ke K)
//  R(17) = Label kanan (KADER Pemantau, No Telp)
//  S(18) = Titik dua kanan
//  T(19) = Nilai kanan — anchor merge T–Y (U–Y = col 20–24, merge ke T)
//  Z(25) = Badge BALITA — merge rows 1–4
//  STATUS GIZI DIHAPUS
// ══════════════════════════════════════════════════════════════
function sheet2(info, pemRows, extra={}) {
  const rows = [], mg = [];
  const TC = 26;  // A–Z (0–25), Status Gizi dihapus
  const M = (r1,c1,r2,c2) => mg.push({s:{r:r1,c:c1},e:{r:r2,c:c2}});

  // ── helper border kosong untuk merge area ────────────────
  const bThin = { style:'thin', color:{ rgb:BLK } };
  const mkEmpty = () => ({
    v:'', t:'s',
    s:{
  ...
  for(let i = 0; i < 4; i++){
    ...
    // Kiri: A=label, B=titik dua, C=nilai anchor, D–H=merge kosong
    row[0] = PLN(leftLabels[i], {sz:10});
    row[1] = PLN(':', {ha:'center'});
    row[2] = PLN(leftValues[i], {bold:true});
    for(let j=3; j<=7; j++) row[j] = mkEmpty();
    M(ri, 2, ri, 7);   // merge C–H

    // Tengah: I=label, J=titik dua, K=nilai anchor, L–Q=merge kosong
    row[8]  = PLN(midLabels[i], {sz:10});
    row[9]  = PLN(':', {ha:'center'});
    row[10] = PLN(midValues[i], {bold:true});
    for(let j=11; j<=16; j++) row[j] = mkEmpty();
    M(ri, 10, ri, 16); // merge K–Q

    // Kanan: R=label, S=titik dua, T=nilai anchor, U–Y=merge kosong
    if(rightLabels[i]){
      row[17] = PLN(rightLabels[i], {sz:10});
      row[18] = PLN(':', {ha:'center'});
      row[19] = PLN(rightValues[i], {bold:true});
    }
    for(let j=20; j<=24; j++) row[j] = mkEmpty();
    M(ri, 19, ri, 24); // merge T–Y

    // Badge BALITA di col Z (25) — hanya row pertama, merge vertikal rows 1–4
    if(i === 0){
  ...
  M(1, 25, 4, 25); // merge BALITA vertikal rows 1–4 col Z
  ...
  if(!dataToShow.length){
    ...
    dataToShow.forEach((r, i) => {
      ...
      rows.push([
        ...
        d(String(r.usiaKehamilanLahir||'')),               // H  Usia Kehamilan
        d(String(r.bbl||'')),                              // I  BBL
        d(String(r.pbl||'')),                              // J  PBL
        d(String(r.ukaLahir||'')),                         // K  UKA Lahir
        ...
        d([r.lila,r.lika].filter(Boolean).join('/'),{ha:'center',bg:BLU2}), // U LILA
        d(r.statusNTO||''  ,{ha:'center',bold:true}),    // V  KET
        d(r.asiEksklusif==='1'?'1':r.asiEksklusif==='2'?'2':'',{ha:'center'}), // W
        d(r.vitAFeb==='1'?'1':r.vitAFeb==='2'?'2':'',{ha:'center'}),         // X
        d(r.bukuKIA==='1'?'1':r.bukuKIA==='2'?'2':'',{ha:'center'}),         // Y
        d([r.ketPerkembangan,r.pkat].filter(Boolean).join('/'),{wrap:true}),   // Z
      ]);
    });
  }
