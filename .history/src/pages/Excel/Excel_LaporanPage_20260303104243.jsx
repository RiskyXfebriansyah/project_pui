...
function sheet1(info, keg, asi, gizi) {
  ...
  [
    ...
    rows.push([LBL(l), VAL(String(v)), ...Array(TC-2).fill(cell('',{bdr:false}))]);
    M(ri,1,ri,TC-1);
  });
  rows.push(BL());
  ...
  const ws = makeWs(rows,[5,56,8,9,9,9,9,9,9,9,9,9,9,12],rows.map((_,i)=>i<2?22:18));
  ws['!merges']=mg;
  return ws;
}
...
function sheet2(info, pemRows, extra={}) {
  ...
  const cw = [5,16,18,6,22,11,5,8,7,7,7,24,16,13,18,4,4,12,8,8,10,8,8,7,7,20,12];

  const rh = rows.map((_,i) => {
    if(i===0) return 26;     // title
    if(i>=1&&i<=4) return 18; // header wilayah ↑ sedikit lebih tinggi
    if(i===5) return 8;      // kosong
    if(i===6) return 16;     // nomor
    if(i===7) return 38;     // header kolom ↑ lebih tinggi untuk wrap
    return 22;               // data
  });

  const ws = makeWs(rows, cw, rh);  // assume makeWs defined
  ws['!merges'] = mg;
  return ws;
}
