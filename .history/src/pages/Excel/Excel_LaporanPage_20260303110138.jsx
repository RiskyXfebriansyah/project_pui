...
//  Sheet 1: Catatan Bulanan (gambar pertama - yang salah)
//  Sheet 2: Formulir Pemantauan Pertumbuhan (gambar kedua - yang diinginkan)
//  Sheet 3: Ringkasan
// ============================================================
...
function sheet1(info, keg, asi, gizi) {
  ...
  const ws = makeWs(rows, [5,45,8,9,9,9,9,9,9,9,9,9,9,12], rows.map((_,i)=> i<2?22 : 18));
  ws['!merges'] = mg;
  return ws;
}
...
function sheet2(info, pemRows, extra={}) {
  ...
  for(let i = 0; i < 4; i++){
    ...
    // Badge BALITA (di row 1, col 25-26)
    if (i === 0) {
      row[25] = cell('BALITA', {
    ...
    // Merges
    M(ri, 2, ri, 7);   // nilai kiri (col 2-7)
    M(ri, 10, ri, 16); // nilai tengah (col 10-16)
    M(ri, 19, ri, 23); // nilai kanan (col 19-23)
  }
  
  // Merge badge BALITA vertikal
  M(1, 25, 4, 26);
  ...
  // Data rows
  if (!pemRows || !pemRows.length) {
    const row = Array(TC).fill(EB());
    row[0] = DAT('1', {ha: 'center'});
    row[4] = DAT('Belum ada data pemantauan');
    rows.push(row);
  } else {
    pemRows.forEach((r, i) => {
      const bg = i % 2 === 1 ? GRN5 : WHT;
  ...
  const cw = [
    ...
    7,    // 23 Vit A
    7,    // 24 Buku KIA
    20,   // 25 Perkembangan/PKAT
    12,   // 26 Status Gizi
  ];

  const rh = rows.map((_, i) => {
    if (i === 0) return 26;
    if (i >= 1 && i <= 4) return 20;
    if (i === 5) return 8;
    if (i === 6) return 18;
    if (i === 7) return 40;
    return 22;
...
export async function exportLaporanExcel({ 
  ...
  try {
    ...
    // Sheet 2: Pemantauan Pertumbuhan (YANG INGIN ANDA LIHAT)
    XLSX.utils.book_append_sheet(
      wb, 
