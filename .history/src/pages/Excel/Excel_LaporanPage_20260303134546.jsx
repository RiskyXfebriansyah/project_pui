...
//  Sheet 2: Formulir Pemantauan Pertumbuhan (YANG ANDA INGINKAN)
//  Sheet 3: Ringkasan
// ============================================================
...
// ══════════════════════════════════════════════════════════════
//  SHEET 2 — Formulir Pemantauan Pertumbuhan (GAMBAR KEDUA)
// ══════════════════════════════════════════════════════════════
function sheet2(info, pemRows, extra={}) {
  ...
  // ROWS 1-4: Header wilayah (4 baris)
  ...
  for(let i = 0; i < 4; i++){
    ...
    for(let j=3; j<=7; j++) row[j] = PLN(''); // isi dengan kosong untuk merge nanti

    // Kolom tengah (col 8-16)
    row[8] = PLN(midLabels[i]);
    row[9] = PLN(':');
    row[10] = PLN(midValues[i], {bold:true});
    for(let j=11; j<=16; j++) row[j] = PLN('');

    // Kolom kanan (col 17-23)
    if (rightLabels[i]) {
    ...
    if (i === 0) {
      ...
      row[26] = cell('', {bdr:true, bg:WHT}); // border saja
    } else {
  ...
  // Merge badge BALITA vertikal (row 1-4, col 25-26)
  M(1, 25, 4, 26);
  ...
  // DATA ROWS - Gunakan data dari parameter atau data contoh
  ...
  const rh = rows.map((_, i) => {
    if (i === 0) return 30;      // Judul
    if (i >= 1 && i <= 4) return 22; // Header wilayah
    if (i === 5) return 10;      // Baris kosong
    if (i === 6) return 20;      // Nomor kolom
    if (i === 7) return 45;      // Header kolom
    return 24;                    // Data rows
  });
...
export async function exportLaporanExcel({ 
  ...
  try {
    ...
    // Sheet 2: PEMANTAUAN PERTUMBUHAN (GAMBAR KEDUA)
    XLSX.utils.book_append_sheet(
      wb, 
