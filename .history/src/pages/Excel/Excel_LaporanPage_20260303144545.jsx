...
async function patchXlsxLandscape(arrayBuf, fileName) {
  ...
  for (const sf of sheetFiles) {
    ...
    xml = xml.replace(/<sheetPr[^>]*>[\s\S]*?<\/sheetPr>/g, '');
    ...
    // ── Inject <sheetPr fitToPage="1"> ───────────────────
    // Harus jadi elemen PERTAMA di dalam <worksheet>
    // agar Excel membaca scaling sebagai "Fit to Page"
    xml = xml.replace(
      /(<worksheet[^>]*>)/,
      '$1<sheetPr><pageSetUpPr fitToPage="1"/></sheetPr>'
    );

    // ── Inject pageMargins + pageSetup sebelum </worksheet>
    // TIDAK pakai r:id karena bisa konflik namespace
