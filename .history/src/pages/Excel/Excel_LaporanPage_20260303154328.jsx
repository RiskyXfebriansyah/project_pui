...
function sheet2(info, pemRows, extra={}) {
  ...
  for(let i = 0; i < 4; i++){
    ...
    // A(0) = label kiri, B(1) = titik dua, C(2) = nilai SENDIRI (no horizontal merge)
    row[0] = PLN(leftLabels[i], {sz:10});
    row[1] = PLN(':', {ha:'center', sz:10});
    row[2] = PLN(leftValues[i], {bold:true, sz:10});

    // D–H (col 3–7): setiap baris isi dengan border agar merge vertikal rapi
    for(let j=3; j<=7; j++) row[j] = mkB();

    // I(8) = label tengah, J(9) = titik dua, K(10) = nilai SENDIRI (no horizontal merge)
    row[8]  = PLN(midLabels[i], {sz:10});
    row[9]  = PLN(':', {ha:'center', sz:10});
    row[10] = PLN(midValues[i], {bold:true, sz:10});

    // L–Q (col 11–16): setiap baris isi dengan border agar merge vertikal rapi
    for(let j=11; j<=16; j++) row[j] = mkB();

    // R(17), S(18), T(19): hanya row 1 dan 2 (i=0,1), row 3-4 kosong
    if(i < 2){
      row[17] = PLN(rightLabels[i] || '', {sz:10});
      row[18] = PLN(':', {ha:'center', sz:10});
      row[19] = PLN(rightValues[i] || '', {bold:true, sz:10});
      // U–Y (col 20–24): merge horizontal per baris (row 1 & 2), all border
      for(let j=20; j<=24; j++) row[j] = mkB();
    } else {
      // Row 3 & 4: R–Y semua kosong (akan di-merge jadi 1 area besar)
