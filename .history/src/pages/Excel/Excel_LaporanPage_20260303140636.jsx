...
function sheet1(info, keg, asi, gizi) {
  ...
  [
    ...
    rows.push([
      LBL(l),                                            // col 0 — label
      VAL(String(v), { ha: 'center' }),                  // col 1 — value di B saja
      cell('', { bg:WHT, bdr:true }),                    // col 2 — anchor merge kosong
      ...Array(TC-3).fill(null),                         // col 3–13 — null (bagian merge)
    ]);
    M(ri, 2, ri, TC-1); // merge kolom C sampai N (kosong putih)
  });
  ...
  const ws = makeWs(rows, [22, 45, 8, 9, 9, 9, 9, 9, 9, 9, 9, 9, 9, 12], rows.map((_,i)=> i<2?22 : 18));
  ws['!merges'] = mg;
  return ws;
}
// ══════════════════════════════════════════════════════════════
