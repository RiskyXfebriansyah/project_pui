...
const PAGE_TITLES = {
  ...
  jadwal:'Jadwal Posyandu', laporan:'Laporan',
...
export default function App() {
  ...
  // ✅ Baca role langsung dari user object
  ...
  useEffect(() => {
    if (user) {
      console.log('[App] User logged in:', user.nama, '| Role:', user.role);
    }
  }, [user]);

  // Reset page kalau tidak diizinkan untuk role ini
  useEffect(() => {
  ...
  function handleNav(page) {
    if (!allowed.includes(page)) {
      alert(`⛔ Akses ditolak\n\n"${PAGE_TITLES[page]}" tidak tersedia untuk role ${role}.`);
      return;
    }
    setActivePage(page);
  }
  ...
  // ── Loading awal (baca session localStorage) ─────────────────
  if (authLoading) {
    return (
      ...
      <div style={{ flex:1, display:'flex', flexDirection:'column', minWidth:0, overflow:'hidden' }}>
        ...
        {activePage === 'balita' && (
          <div style={{
            ...
            <input placeholder="🔍  Cari nama, NIK, atau ibu..."
              value={balita.search} onChange={e=>balita.setSearch(e.target.value)}
              style={{ padding:'8px 14px', borderRadius:8, border:'1.5px solid #E5E7EB',
            ...
            <select value={balita.filterDesa} onChange={e=>balita.setFilterDesa(e.target.value)}
              style={{ padding:'8px 12px', borderRadius:8, border:'1.5px solid #E5E7EB',
