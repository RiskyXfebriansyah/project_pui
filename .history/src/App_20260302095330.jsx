...
export default function App() {
  ...
  // ✅ role langsung dari user.role — sudah lowercase dari useAuth
  const role    = user?.role || 'kader';
  const allowed = ALLOWED_PAGES[role] || ALLOWED_PAGES.kader;

  useEffect(() => {
    if (user) {
      console.log('[App] User:', user.nama, '| role:', user.role, '| allowed:', ALLOWED_PAGES[user.role]);
    }
  }, [user]);
  ...
  function handleNav(page) {
    if (!allowed.includes(page)) {
      alert(`⛔ Akses ditolak — halaman "${PAGE_TITLES[page]}" tidak tersedia untuk role ${role}`);
      return;
    }
    setActivePage(page);
  }
