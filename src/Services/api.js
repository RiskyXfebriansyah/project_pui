...
//const BASE_URL = 'http://192.168.3.248:2226'; KANTOR

//RUMAH
// const BASE_URL ='http://192.168.1.10:2226';

// //kiki hostpot hp
// const BASE_URL ='http://172.20.10.3:2226';

const BASE_URL ='http://192.168.100.20:2226';  //wifi rumah 
...
export const AuthAPI = {
  ...
  // ✅ PINDAH KE DALAM object AuthAPI, ganti ; jadi ,
  forgotPassword: async (email, passwordBaru) => {
    try {
      ...
      return res.json();
    } catch (err) { console.error('[api] ForgotPassword error:', err); throw err; }
  },
};
...
export const BalitaAPI = {
  ...
  update: (id, data) => apiFetch(`/api/Balita/Update?id=${id}`, {  // ← NEW
    method: 'PUT',
    body:   JSON.stringify(data),
  }),
...
export const PosyanduAPI = {
  getAll: ()     => apiFetch('/api/Posyandu/GetAll'),
  create: (data) => apiFetch('/api/Posyandu/Create', {
    method: 'POST',
    body:   JSON.stringify(data),
  }),
  delete: (id)   => apiFetch(`/api/Posyandu/Delete?id=${id}`, { method: 'DELETE' }),
};
