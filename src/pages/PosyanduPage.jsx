...
export function PosyanduPage({
  posyanduList = [],
  balitaList = [],          // ← BARU: data balita untuk hitung sinkron
 ...
 // ── ENRICH: Langsung pakai totalBalita & stunting dari API response ──────
const enrichedList = posyanduList.map(p => ({
  ...p,
  totalBalita: p.totalBalita ?? 0,
  stunting: p.stunting ?? 0,
}));
  ...
  return (
    <div style={{ padding: 24, fontFamily: font, minHeight: '100vh', background: C.bg }}>
      ...
      {isLoading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(280px,1fr))', gap: 16 }}>
          {/* ← enrichedList: totalBalita & stunting sudah dihitung dari balitaList */}
          {enrichedList.map(p => (
            <PosyanduCard
              key={p.id}
              p={p}
