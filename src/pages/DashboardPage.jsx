...
// ============================================================
//  DashboardPage — dengan tombol Grafik KMS di setiap role
//  Props baru: onShowKMS(balitaObj)
// ============================================================
...
const DASH_CSS = `
  ...
  .banner-actions { margin-left:auto; display:flex; gap:8px; position:relative; z-index:1; flex-wrap:wrap; }
  .banner-btn {
  ...
  .banner-btn-kms {
    ...
    border-radius:12px; padding:8px 16px; color:#818CF8; cursor:pointer;
    font-size:12px; font-weight:600; font-family:'DM Sans',sans-serif;
    display:flex; align-items:center; gap:6px; transition:all 0.2s;
  }
  .banner-btn-kms:hover { background:rgba(99,102,241,0.25); transform:translateY(-1px); }
  ...
  .kms-shortcut-icon {
    ...
    background:rgba(129,140,248,0.15); border:1.5px solid rgba(129,140,248,0.3);
    display:flex; align-items:center; justify-content:center; flex-shrink:0;
  }
  .kms-shortcut-btn {
    background:rgba(129,140,248,0.2); border:1px solid rgba(129,140,248,0.4);
    border-radius:10px; padding:8px 16px; color:#A5B4FC; cursor:pointer;
    font-size:12px; font-weight:700; font-family:'DM Sans',sans-serif;
    transition:all 0.2s; margin-left:auto; flex-shrink:0;
  }
  .kms-shortcut-btn:hover { background:rgba(129,140,248,0.3); }
  .kms-item-btn {
    background:rgba(99,102,241,0.1); border:1px solid rgba(99,102,241,0.25);
    border-radius:7px; padding:4px 8px; color:#6366F1; cursor:pointer;
    font-size:10px; font-weight:700; font-family:'DM Sans',sans-serif;
    transition:all 0.15s; flex-shrink:0;
  }
  .kms-item-btn:hover { background:rgba(99,102,241,0.2); }
...
// ── KMS Shortcut Banner (shared semua role) ──────────────────
function KMSShortcut({ balitaList, onShowKMS }) {
  // Pilih balita stunting/risiko terakhir untuk di-highlight, atau balita pertama
  const highlight = balitaList?.find(b => {
  ...
  return (
    <div className="kms-shortcut anim d2">
      <div className="kms-shortcut-icon">📈</div>
      <div>
        <div style={{fontFamily:'Sora,sans-serif',color:'#fff',fontWeight:800,fontSize:14,marginBottom:4}}>
          Grafik KMS WHO
        </div>
        <div style={{color:'rgba(165,180,252,0.6)',fontSize:11}}>
          Pantau tumbuh kembang balita: BB/U · TB/U · LK/U
        </div>
      </div>
      <button
        className="kms-shortcut-btn"
        onClick={() => onShowKMS(highlight || null)}
      >
        📊 Buka Grafik
      </button>
    </div>
...
// ── Admin ────────────────────────────────────────────────────
function AdminDashboard({ statistik, upcomingJadwal, balitaList, onNav, onShowKMS }) {
  injectCSS();
  const pie = [
    ...
    <div className="dash-root" style={{padding:24}}>
      <div className="dash-banner banner-admin anim">
        ...
        <div className="banner-actions">
          ...
          <button className="banner-btn-kms" onClick={()=>onShowKMS(balitaList?.[0]||null)}>
            📈 Grafik KMS
          </button>
        </div>
      </div>
      ...
      {/* KMS shortcut */}
      <KMSShortcut balitaList={balitaList} onShowKMS={onShowKMS}/>
...
// ── Bidan ────────────────────────────────────────────────────
function BidanDashboard({ statistik, upcomingJadwal, balitaList, onNav, onShowKMS }) {
  injectCSS();
        ...
        <Panel title="⚠️ Balita Perlu Perhatian" action="Lihat Semua" onAction={()=>onNav('stunting')}>
          {prioritas.length===0
            ...
            : prioritas.map(b=>{
              ...
              return (
                <div key={b.id} className="priority-item" onClick={()=>onShowKMS(b)}>
                  ...
                  <button className="kms-item-btn" onClick={e=>{e.stopPropagation();onShowKMS(b);}}>📈</button>
                </div>
              );
            })
          }
...
// ── Kader ────────────────────────────────────────────────────
function KaderDashboard({ statistik, upcomingJadwal, balitaList, onNav, onShowKMS }) {
  injectCSS();
  ...
  return (
    <div className="dash-root" style={{padding:24}}>
      <div className="dash-banner banner-kader anim">
        ...
        <div style={{marginLeft:'auto',position:'relative',zIndex:1,display:'flex',gap:8,alignItems:'center'}}>
          <button className="banner-btn-kms" onClick={()=>onShowKMS(balitaList?.[0]||null)}>
            📈 Grafik KMS
          </button>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:16}}>
        <Panel title="📋 Tugas Kader">
          {[
            {done:false,task:'Cek jadwal posyandu bulan ini',      action:()=>onNav('jadwal')},
            ...
            {done:false,task:'Laporan balita normal, stunting, risiko', action:()=>onNav('pemantauan')},
          ].map((t,i)=>(
...
function JadwalSection({ upcomingJadwal, onNav }) {
  return (
    <Panel title="📅 Jadwal Mendatang" action="Lihat Semua" onAction={()=>onNav('jadwal')} style={{marginBottom:0}}>
...
// ── Export ───────────────────────────────────────────────────
export default function DashboardPage({ statistik, upcomingJadwal, balitaList, onNav, role, onShowKMS }) {
  if (role==='admin') return <AdminDashboard statistik={statistik} upcomingJadwal={upcomingJadwal} balitaList={balitaList} onNav={onNav} onShowKMS={onShowKMS}/>;
  if (role==='bidan') return <BidanDashboard statistik={statistik} upcomingJadwal={upcomingJadwal} balitaList={balitaList} onNav={onNav} onShowKMS={onShowKMS}/>;
  return <KaderDashboard statistik={statistik} upcomingJadwal={upcomingJadwal} balitaList={balitaList} onNav={onNav} onShowKMS={onShowKMS}/>;
}