// ============================================================
//  App.jsx — dengan integrasi GrafikKMSPage
// ============================================================
...
import GrafikKMSPage   from './pages/GrafikKMSPage';   // ← tambahkan
...
const PAGE_TITLES = {
  ...
  kms:        'Grafik KMS',             // ← tambahkan
};

const ALLOWED_PAGES = {
  admin: ['dashboard','balita','pemantauan','stunting','jadwal','laporan','posyandu','pengguna','kms'],
  bidan: ['dashboard','balita','pemantauan','stunting','jadwal','laporan','kms'],
  kader: ['dashboard','balita','pemantauan','jadwal','laporan','kms'],
};

export default function App() {
  const { user, isLoading: authLoading, error: authError, login, logout, forgotPassword } = useAuth();
  ...
  const [selectedBalita, setSelectedBalita] = useState(null); // ← untuk KMS
  ...
  function handleNav(page) {
    ...
    // Kalau bukan KMS, clear selectedBalita
    if (page !== 'kms') setSelectedBalita(null);
    setActivePage(page);
  }

  // ← handler baru: buka KMS untuk balita tertentu (atau null = demo)
  function handleShowKMS(balitaObj) {
    setSelectedBalita(balitaObj || null);
    setActivePage('kms');
  }
  ...
  if (!user) return <LoginPage onLogin={login} onForgot={forgotPassword} error={authError} />;
  ...
  function renderDashboard() {
    return (
      <DashboardPage
        ...
        posyandu={posyandu.posyanduList}
        onNav={handleNav}
        onShowKMS={handleShowKMS}   // ← teruskan handler KMS
      />
    );
  }
      ...
      case 'balita':
        return (
          <BalitaPage
            ...
            currentUser={user}
            onEditBalita={balita.editBalita}
            onShowKMS={handleShowKMS}   // ← opsional: tombol KMS per balita
          />
        );
      ...
      case 'pemantauan':
        return (
          <PemantauanPage
            ...
            onShowKMS={handleShowKMS}   // ← opsional
          />
        );
